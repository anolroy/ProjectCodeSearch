...
'   If Not bData Then
'      MsgBox "No Data has found.", vbInformation + vbOKOnly, "No DATA"
'      Unload Me
'      Exit Sub
'   End If
'   frmPurchaseExpense.flxPI.TextMatrix(frmPurchaseExpense.flxPI.Row, frmPurchaseExpense.flxPI.Col) = _
'                              flxSupplier.TextMatrix(flxSupplier.Row, flxSupplier.Col)
'   If Me.Caption = "SUPPLIER ACCOUNT" Then
'      frmPurchaseExpense.flxPI.TextMatrix(frmPurchaseExpense.flxPI.Row, 12) = _
'                              "T" & flxSupplier.TextMatrix(flxSupplier.Row, 3)
'      frmPurchaseExpense.nTaxCode = TaxRate(CInt(flxSupplier.TextMatrix(flxSupplier.Row, 3)))
'      frmPurchaseExpense.flxPI.TextMatrix(frmPurchaseExpense.flxPI.Row, 6) = _
'                              flxSupplier.TextMatrix(flxSupplier.Row, 2)
'   End If
...
Private Sub flxSupplier_DblClick()
   If Not bData Then Unload Me
   cmdSel_Click
End Sub

Private Sub Form_Load()
'   Me.Top = frmPurchaseExpense.Top + frmPurchaseExpense.flxPI.Top + (285 * frmPurchaseExpense.flxPI.Rows) + 1000
   Me.Left = frmPurchaseExpense.iFlxLeft
   
   bData = True            'DATA has found
   ...
   Else
     ' Try to Connect - Will Throw an Exception if it Fails
      If oWS.Connect(szDataPath, sageUserName, sagePassword, "Example") Then
         ...
         flxSupplier.Clear
         flxSupplier.TextMatrix(0, 0) = "Proj. Ref."
         flxSupplier.TextMatrix(0, 1) = "Project Name"
...
'   MsgBox "The SDO generated the following error: " & oSDO.LastError.text
   ...
   bData = False              'No data has found in the SAGE
'   Unload Me
End Sub
   ...
   Else
     ' Try to Connect - Will Throw an Exception if it Fails
      If oWS.Connect(szDataPath, sageUserName, sagePassword, "Example") Then
         ...
         flxSupplier.Clear
         flxSupplier.TextMatrix(0, 0) = "Tax Code"
         flxSupplier.TextMatrix(0, 1) = "Tax Rate"
...
'
      flxSupplier.TextMatrix(0, 0) = "Unit ID"
      flxSupplier.TextMatrix(0, 1) = "Tenant Company Name"
'
      rRow = 1
   ...
   Else
     ' Try to Connect - Will Throw an Exception if it Fails
      If oWS.Connect(szDataPath, sageUserName, sagePassword, "Example") Then
         ...
         flxSupplier.Clear
         flxSupplier.TextMatrix(0, 0) = "Dept. ID"
         flxSupplier.TextMatrix(0, 1) = "Department Name"
   ...
   Else
     ' Try to Connect - Will Throw an Exception if it Fails
      If oWS.Connect(szDataPath, sageUserName, sagePassword, "Example") Then
         ...
         flxSupplier.Clear
         flxSupplier.TextMatrix(0, 0) = "A/C ID"
         flxSupplier.TextMatrix(0, 1) = "Name"
         
         Dim rRow As Integer
         For rRow = 1 To oPurchaseRecord.Count
            flxSupplier.TextMatrix(rRow, 0) = CStr(oPurchaseRecord.Fields.Item("ACCOUNT_REF").Value)
            flxSupplier.TextMatrix(rRow, 1) = CStr(oPurchaseRecord.Fields.Item("NAME").Value)
            flxSupplier.TextMatrix(rRow, 2) = CStr(oPurchaseRecord.Fields.Item("DEF_NOM_CODE").Value)
            flxSupplier.TextMatrix(rRow, 3) = CStr(oPurchaseRecord.Fields.Item("DEF_TAX_CODE").Value)
...
Error_Handler:
   ...
   MsgBox "(pcm_sage_supplier_ac) The SDO generated the following error: " & oSDO.LastError.text
