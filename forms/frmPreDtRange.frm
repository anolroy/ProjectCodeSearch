...
Begin VB.Form frmPreDtRange 
   ...
   End
End
Attribute VB_Name = "frmPreDtRange"
...
Private Sub txtToDate_GotFocus()
   If Len(txtToDate.text) < 10 Then txtToDate.text = Format(Date, "dd/mm/yyyy")
...
Private Sub txtToDate_LostFocus()
    If txtToDate.text <> "" Then TextBoxFormatDate txtToDate
End Sub
...
Private Sub txtFromDate_GotFocus()
   If Len(txtFromDate.text) < 10 Then txtFromDate.text = Format(Date, "dd/mm/yyyy")
...
Private Sub txtFromDate_LostFocus()
    If txtFromDate.text <> "" Then TextBoxFormatDate txtFromDate
End Sub
...
Private Sub ConfigFlxProperties()
   ...
   With flxProperties
      ...
      .Cols = 4
      .RowHeight(0) = 0
      .ColWidth(0) = Label2(0).Left - .Left + 100 '200                 '"X"
      .ColWidth(1) = Label2(1).Left - Label2(0).Left 'Property ID
      .ColWidth(2) = Label2(2).Left - Label2(1).Left - 100 'Property Name
...
Private Sub LoadProperty(adoConn As ADODB.Connection)
    ...
    Else
        szSQL = "SELECT   PROPERTYID, PROPERTYNAME, ClientID " & _
           "FROM     PROPERTY ORDER BY PROPERTYID;"
    End If
   adoRst.Open szSQL, adoConn, adOpenStatic, adLockReadOnly
...
Private Sub cmdSCYRRPrint_Click()
    ...
    If ListOfProperties2 = "" Then
        MsgBox "Please select Property", vbInformation, "Warning"
        Exit Sub
    End If
    strSelectionFormula = "{RentAnalysis.RRStatus} = 'Y' and {LeaseDetails.Status} "
    strSelectionFormula = strSelectionFormula & " AND {Property.PropertyID} in [" & ListOfProperties2 & "]"
    If Option2.Value = True Then
        strSelectionFormula = strSelectionFormula & " AND {RentAnalysis.RentReviewDate} >=#" & CDate(CStr(cmbPeriodFrom.Column(2))) & "# AND {RentAnalysis.RentReviewDate}<=#" & CDate(cmbPeriodTo.Column(3)) & "#"
    Else
        strSelectionFormula = strSelectionFormula & " AND {RentAnalysis.RentReviewDate} >=#" & CDate(txtFromDate.text) & "# AND {RentAnalysis.RentReviewDate}<=#" & CDate(txtToDate.text) & "#"
    End If
    ...
    Report.ParameterFields(3).AddCurrentValue txtClientList.text
...
'              szPropSrc & _
'              "P.TRAN_DATE >= #" & Format(txtFromDate.text, "DD MMMM YYYY") & "# AND " & _
'              "P.TRAN_DATE <= #" & Format(txtToDate.text, "DD MMMM YYYY") & "# " & _
...
'              "P.PDate <= #" & Format(txtToDate.text, "DD MMMM YYYY") & "# " & _
'           "GROUP BY P.BankCode;"
'   adoRst.Open szSQL, adoConn, adOpenStatic, adLockReadOnly
'
'   While Not adoRst.EOF
...
'              szFundPA & _
'              "P.PDate >= #" & Format(txtFromDate.text, "DD MMMM YYYY") & "# AND " & _
'              "P.PDate <= #" & Format(txtToDate.text, "DD MMMM YYYY") & "#;"
'
'   adoRst.Open szSQL, adoConn, adOpenStatic, adLockReadOnly
'
'   If adoNL.Fields.Item("DrCr").Value = "Cr" Then
...
'                  "P1.PDate <= #" & Format(txtToDate.text, "DD MMMM YYYY") & "# " & _
...
'              "D.IssueDate >= #" & Format(txtFromDate.text, "DD MMMM YYYY") & "# AND " & _
'              "D.IssueDate <= #" & Format(txtToDate.text, "DD MMMM YYYY") & "# AND " & _
...
'              szFundSA & _
'              "R.RDate >= #" & Format(txtFromDate.text, "DD MMMM YYYY") & "# AND " & _
'              "R.RDate <= #" & Format(txtToDate.text, "DD MMMM YYYY") & "# " & _
'           "GROUP BY R.BankCode;"
'   adoRst.Open szSQL, adoConn, adOpenStatic, adLockReadOnly
'
'   While Not adoRst.EOF
...
'              szFundSA & _
'              "R.RDate >= #" & Format(txtFromDate.text, "DD MMMM YYYY") & "# AND " & _
'              "R.RDate <= #" & Format(txtToDate.text, "DD MMMM YYYY") & "#;"
''Debug.Print szSQL
'   adoRst.Open szSQL, adoConn, adOpenStatic, adLockReadOnly
'
'   If adoNL.Fields.Item("DrCr").Value = "Cr" Then
...
'                  szFundSR & _
'                  "R1.RDate >= #" & Format(txtFromDate.text, "DD MMMM YYYY") & "# AND " & _
'                  "R1.RDate <= #" & Format(txtToDate.text, "DD MMMM YYYY") & "# " & _
...
'           "WHERE " & _
'              szFundBank & _
'              "B.TRAN_DATE >= #" & Format(txtFromDate.text, "DD MMMM YYYY") & "# AND " & _
'              "B.TRAN_DATE <= #" & Format(txtToDate.text, "DD MMMM YYYY") & "# " & _
