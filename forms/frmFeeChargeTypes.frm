...
Begin VB.Form frmChargeTypes 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Fee Charge Types"
   ClientHeight    =   12045
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   22410
   Icon            =   "frmFeeChargeTypes.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ...
   Begin VB.Frame fraDemandType 
      ...
      Begin VB.Label Label1 
         ...
         BackColor       =   &H00FFDFC0&
         BackStyle       =   0  'Transparent
         Caption         =   "Nominal code for Fee Charge Types:"
         BeginProperty Font 
            Name            =   "Myriad Web"
            Size            =   9
            Charset         =   0
      ...
      Begin VB.Label Label1 
         ...
         BackColor       =   &H00FFDFC0&
         BackStyle       =   0  'Transparent
         Caption         =   "Fee Charge Type Category:"
         BeginProperty Font 
            Name            =   "Myriad Web"
            Size            =   9
            Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmChargeTypes"
...
'            "FROM ChargeTypes,SecondaryCode A " & _
'            "WHERE CSTR(ChargeTypes.CategoryCode)=A.Code AND  PrimaryCode= 'DCTG' AND ID = " & szDemandID & ""
            
'    szSQL = "SELECT ID,CL.ClientName,CL.ClientID,P.PropertyName,P.PropertyID, FeeType, FeeIC, FeeSagePrefix," & _
'                  "FeeNCVat, FeeNNVat, FeeNCTotal, FeeNNTotal, TransactionType, " & _
'                  "CategoryCode, PaymentDates, RecoverableExp, Value as CategoryName,GroupID,FeeNCAmt,FeeNNAmt " & _
'                "FROM ChargeTypes C,SecondaryCode S,Property P,Client CL where CL.ClientID=C.ClientID  " & _
'                "and P.ClientID=CL.ClientID AND P.PropertyID=C.PropertyID AND  S.Code=cstr(C.CategoryCode) " & _
'                "AND  S.PrimaryCode='DCTG' AND ID = " & szDemandId & " ORDER BY ID"
  szSQL = "SELECT ID,CL.ClientName,CL.ClientID,P.PropertyName,P.PropertyID, FeeType, FeeIC, FeeSagePrefix," & _
                  "FeeNCVat, FeeNNVat, FeeNCTotal, FeeNNTotal, TransactionType, " & _
                  "PaymentDates, RecoverableExp,GroupID,FeeNCAmt,FeeNNAmt " & _
                "FROM ChargeTypes C,Property P,Client CL where CL.ClientID=C.ClientID  " & _
...
'    Exit Sub
   ...
   If IsNull(adoRst!FeeNCAmt) = False Then
        txtDemandTypeNCAmt.text = adoRst!FeeNCAmt
   End If
   If IsNull(adoRst!FeeNNAmt) = False Then
            txt1.text = adoRst!FeeNNAmt
   End If
   txtPrefix.text = IIf(IsNull(adoRst!FeeSagePrefix), "", IIf(adoRst!FeeSagePrefix = "NULL", "", adoRst!FeeSagePrefix))
...
KatchErr:
   ShowMsgInTaskBar "Could not load fee Charges due to to some error", "Y", "N"
   adoConn.Close
   Set adoConn = Nothing
End Sub
...
'            adoConn.Open getConnectionString
'            adoRst.Open "Select StatementTemplate from ChargeTypes where PropertyID='" & txtProperty.Tag & "' AND (StatementTemplate<>'' or StatementTemplate is not null) order by ID DESC", adoConn, adOpenStatic, adLockReadOnly
'            If Not adoRst.EOF Then
         ...
         If txtDemandCategoryCode.text = "1" Then                              'Rent Charge
            szSQL = "SELECT PropertyName " & _
                    "FROM (SELECT DISTINCT U.PropertyID, P.PropertyName " & _
                          "FROM   ChargeTypes AS T, LRentCharges AS R, " & _
...
'Debug.Print szSQL
         ...
         If txtDemandCategoryCode.text = "2" Then                              'Service Charge
            szSQL = "SELECT PropertyName " & _
                    "FROM (SELECT DISTINCT U.PropertyID, P.PropertyName " & _
                          "FROM   ChargeTypes AS T, LServiceCharges AS S, " & _
                               "LeaseDetails AS L, Units AS U, Property AS P " & _
...
'Debug.Print szSQL
         ...
         If txtDemandCategoryCode.text = "3" Then                              'Insurance Charge
            szSQL = "SELECT PropertyName " & _
                    "FROM (SELECT DISTINCT U.PropertyID, P.PropertyName " & _
                          "FROM   ChargeTypes AS T, LInsuranceCharges AS I, " & _
                               "LeaseDetails AS L, Units AS U, Property AS P " & _
...
Private Sub cmdDelete_Click()
   ...
   Dim adoRst As New ADODB.Recordset
   If szDemandId = "" Then
       MsgBox "You must select a Fee charge type to delete!", vbOKOnly + vbCritical, "No Fee charge type selected"
       Exit Sub
   End If
   ...
   adoConn.Open getConnectionString
   'Mark 1
   If isChargeTypeIDUsedinagrement(adoConn, szDemandId) = True Then
       MsgBox "You cannot delete this charge type because it is linked to an agreement. " & _
        "To delete it, please remove it from the agreement", vbOKOnly + vbCritical, "Cannot delete Fee charge type"
       Exit Sub
   End If
   'End of addition
   Response = MsgBox("Are you sure you want to delete Fee charge type: " & szDemandType, vbYesNo + vbQuestion, "Delete")
   ...
   szSQL = "SELECT * " & _
            "FROM ChargeTypes WHERE ID = " & szDemandId & ""
   adoRst.Open szSQL, adoConn, adOpenDynamic, adLockOptimistic
   ...
   MsgBox "Selected Fee charge type has been deleted", vbInformation, "Deleted"
   adoConn.Close
   Set adoConn = Nothing

   Call EmptyBoxes
End Sub
Private Function isChargeTypeIDUsedinagrement(adoConn As ADODB.Connection, szDemandId As String) As Boolean
    ...
    rsCheck.Open "Select * from tlbAgreement agr where agr.CHARGE_TYPE=" & szDemandId & "", adoConn, adOpenStatic, adLockReadOnly
    If Not rsCheck.EOF Then
        isChargeTypeIDUsedinagrement = True
        Exit Function
    End If
    rsCheck.Close
End Function
Private Function isDemandIdUsedinTrans(adoConn As ADODB.Connection, szDemandId As String) As Boolean
    'Written by anol 13 Apr 2015
    'issue 0000554: Fee charge Types can not be deleted when being used in the system
    'This function shall check demandtype before delete.
    'If demand type is used in transaction,this function won't let user to delete demand type.
    'Mark 1 When salia give you full spec impletement this method fully
...
Private Sub cmdDemandCategory_Click()
    picClient.Left = 2880
    picClient.Top = 4230
    strCommandSource = "Fee charge Category"
       ...
       If optCopyDemandType.Value Then 'copy demand type for each of properties
            ...
            For j = 0 To UBound(tmpDemand)
                    For i = 1 To flxProperty1.Rows - 1
                      If flxProperty1.TextMatrix(i, 0) = "X" Then
                           iCount = iCount + 1
                           szSQL = "SELECT MAX(ID) FROM ChargeTypes"
                           adoRst.Open szSQL, adoConn, adOpenStatic, adLockReadOnly
                           ...
                           szSQL = "SELECT ID, Type, Prefix, NominalCodeforAmount, InvCrd, " & _
                                       ...
                                       "CategoryCode, PaymentDates, GroupID, DemandReportName, " & _
                                       "Spare1, PropertyID, EmailInvoiceTemplate, StatementTemplate " & _
                                    "FROM ChargeTypes where ID=" & tmpDemand(j) & ""
...
'                           If chkGroup.Value = 1 Then adoRST!GroupID = txtGroup.text
   ...
   Else 'copy Demand Template
        ...
        Next i
         ...
         For i = 1 To flxDemandTypeList.Rows - 1
              If flxDemandTypeList.TextMatrix(i, 0) = "X" Then
                     szSQL = "SELECT ID, Type, Prefix, NominalCodeforAmount, InvCrd, " & _
                                       ...
                                       "CategoryCode, PaymentDates, GroupID, DemandReportName, " & _
                                       "Spare1, PropertyID, EmailInvoiceTemplate, StatementTemplate " & _
                                    "FROM ChargeTypes where ID=" & flxDemandTypeList.TextMatrix(i, 1) & ""
                    rsSourceDemand.Open szSQL, adoConn, adOpenDynamic, adLockOptimistic
         ...
         For i = 1 To flxDemandTypeList2.Rows - 1
              If flxDemandTypeList2.TextMatrix(i, 0) = "X" Then
                    adoConn.Execute "Update ChargeTypes set DemandReportName = '" & rsSourceDemand("DemandReportName").Value & "'," & _
...
'   connect to database
   ...
   For iProp = 1 To flxProperty.Rows - 1
      If flxProperty.TextMatrix(iProp, 0) = "X" Then
         szDes = szDes & "ChargeTypes.PropertyID = '" & flxProperty.TextMatrix(iProp, 1) & "'"
         szDes = szDes & " OR "
      End If
   Next iProp
   ...
   szSQL = "SELECT ID, TYPE FROM ChargeTypes WHERE (" & szDes & ");"
...
'   connect to database
   ...
   For iProp = 1 To flxProperty.Rows - 1
      If flxProperty1.TextMatrix(iProp, 0) = "X" Then
         szDes = szDes & "ChargeTypes.PropertyID = '" & flxProperty1.TextMatrix(iProp, 1) & "'"
         szDes = szDes & " OR "
      End If
   Next iProp
    'Fixed by anol 20170326
...
'   On Error Resume Next
   Dim i As Integer
   If txtPrefix.text = "" Then
      MsgBox "You must enter Demand Prefix", vbOKOnly + vbCritical, "Fee charge Type"
      txtPrefix.SetFocus
      Exit Sub
   End If
   If txtDemandTypeNCAmt.text = "" Then
...
'   End If
   ...
   Set adoRst = Nothing
    ...
    szSQL = "SELECT D.ID, D.Type, D.PropertyID, " & _
           ...
           "FROM (ChargeTypes AS D LEFT JOIN Property AS P ON " & _
...
'   End If
   ...
   If txtType.text = "" Then
      MsgBox "You must enter a Fee charge Type", vbOKOnly + vbCritical, "Fee charge Type"
      txtType.SetFocus
      Exit Sub
   End If
   If txtPrefix.text = "" Then
      MsgBox "You must enter a Fee charge Prefix", vbOKOnly + vbCritical, "Fee charge Type"
      txtPrefix.SetFocus
      Exit Sub
   End If
'   If txtDemandTypeNCAmt.text = "" Then
...
'   End If
   ...
   szSQL = "SELECT ID, FeeType, FeeIC, FeeSagePrefix, FeeNCAmt, FeeNNAmt, " & _
                  "FeeNCVat, FeeNNVat, FeeNCTotal, FeeNNTotal, TransactionType, " & _
                  "CategoryCode, PaymentDates, RecoverableExp, GroupID,FeeNCAmt,FeeNNAmt,PropertyID,clientID,LastModifiedBy,LastModifiedDate,CreatedBy,CreatedDate " & _
                  "FROM ChargeTypes C where ID=" & szDemandId & ""
   ...
   szSQL = "SELECT T.CAName, S.Value, T.Code AS NCode, T.Name AS NName, T.ClientID, T.CAFixed AS Fixed," _
            & "IIF(T.CAPosting, 'YES', 'NO') AS P, T.CAType AS Type, T.CADisOrder FROM NominalLedger AS T," _
            & "SecondaryCode AS S,Property  WHERE Property.ClientID=T.ClientID AND T.CAType = S.Code AND " _
            & "S.PrimaryCode = 'CAT' AND Property.PropertyID = '" & txtProperty.Tag & "' ORDER By t.CADisOrder"
   rsCheck.Open szSQL, adoConn, adOpenDynamic, adLockOptimistic
   Dim x1 As String
   Dim x2 As String
   Dim x3 As String
...
'  Thats why I am saving a charecter. I have changed the field as not required (03/09/2009).
   adoRst!FeeIC = "X"
   adoRst!FeeType = txtType.text
   adoRst!FeeSagePrefix = txtPrefix.text
'   adoRst!FeeNCAmt = txtDemandTypeNCAmt.text
'   adoRst!FeeNNAmt = txt1.text
   adoRst!FeeNCVat = x3 ''cboDemandTypeNCvat.text
   adoRst!FeeNNVat = x4 '' txt2.text
   adoRst!FeeNCTotal = x1 'cboDemandTypeNCTotal.text & ""
   adoRst!FeeNNTotal = x2 ''txt3.text
'   adoRst!prefix = "NULL"
'   adoRst!CategoryCode = txtDemandCategoryCode.text
   If optAuto.Value Then
      adoRst!PaymentDates = CByte(255)
...
'   flxDemandTypes.row = intTopRow
   ...
   cmdClientFilter.Enabled = True
   If EditMode = False Then
        MsgBox "Your New Fee charge Type details have been saved.", vbInformation, "Saved"
   Else
        MsgBox "Edited charge type details have been saved.", vbInformation, "Saved"
   End If
   Exit Sub
...
'                FocusControl cmdBank
        ...
        ElseIf strCommandSource = "Fee charge Category" Then
...
Private Sub flxDemandTypes_RowColChange()
   With flxDemandTypes
      ...
      szExistingProperty = txtProperty.Tag
      Me.Caption = "Fee charge Types - " & txtProperty.text
   End With
...
Private Sub ConfigFlxDemandTypes()
    ...
    With flxDemandTypes
        ...
        .ColWidth(8) = 9000 'Label1(30).Left - Label1(29).Left            'FeeType
        .ColWidth(9) = 0 'Label1(31).Left - Label1(30).Left             'Charge Category Name
        ...
        .ColWidth(15) = 0 'Label1(32).Left - Label1(31).Left                                             'Nominal Code
'        .ColAlignment(15) = vbAlignLeft
        .ColWidth(16) = 0 'Label1(33).Left - Label1(32).Left                                            'Nominal Name
    End With
End Sub
...
'    Dim szSQL1 As String
        If txtClientList.Tag = "ALL" And txtPropertyList.Tag = "ALL" Then
                  ...
                  "IIF(ISNULL(P.ClientID), 'All Clients', P.ClientID) AS ClientID, " & _
                  "IIF(ISNULL(P.ClientID), '', C.ClientName) AS ClientName " & _
                  "FROM (ChargeTypes AS D  INNER JOIN Property AS P ON " & _
                      "D.PropertyID = P.PropertyID ) INNER JOIN Client AS C ON P.ClientID = C.ClientID " & _
                " ORDER BY ClientName, PropertyName, D.FEEType, D.ID;"
...
'    szSQL = "SELECT ID,CL.ClientName,CL.ClientID,P.PropertyName,P.PropertyID, FeeType, FeeIC, FeeSagePrefix, FeeNCAmt, FeeNNAmt, " & _
'                  "FeeNCVat, FeeNNVat, FeeNCTotal, FeeNNTotal, TransactionType, " & _
...
'                "and P.ClientID=CL.ClientID AND P.PropertyID=C.PropertyID AND  S.Code=cstr(C.CategoryCode) AND  S.PrimaryCode='DCTG' ORDER BY ID"
   ...
   With adoRst.Fields
      For i = 0 To adoRst.RecordCount - 1
         ...
         flxDemandTypes.TextMatrix(i + 1, 7) = IIf(IsNull(.Item("FeeSagePrefix")), "", .Item("FeeSagePrefix"))
         flxDemandTypes.TextMatrix(i + 1, 8) = IIf(IsNull(.Item("FeeType")), "", .Item("FeeType"))
...
'         flxDemandTypes.TextMatrix(i + 1, 15) = IIf(IsNull(.Item("FeeNCAmt")), "", .Item("FeeNCAmt"))
'         flxDemandTypes.TextMatrix(i + 1, 16) = IIf(IsNull(.Item("FeeNNAmt")), "", .Item("FeeNNAmt"))
