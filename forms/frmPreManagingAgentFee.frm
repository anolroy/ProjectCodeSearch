...
Begin VB.Form frmPreManagingAgentFee 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Managing Agent Fee"
   ClientHeight    =   8280
   ClientLeft      =   1125
   ClientTop       =   1935
   ClientWidth     =   7875
   ...
   EndProperty
   Icon            =   "frmPreManagingAgentFee.frx":0000
   LinkTopic       =   "Form2"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ScaleHeight     =   5715.004
   ScaleMode       =   0  'User
   ScaleWidth      =   7395.036
   ...
   Begin VB.Frame Frame3 
      ...
      Top             =   2250
      Width           =   6585
      Begin VB.OptionButton optPaymentDate 
   ...
   End
End
Attribute VB_Name = "frmPreManagingAgentFee"
...
Private Sub flxClient_Click()
        ...
        End If
        If sTextBox = "3" Then
                txtFundName.text = flxClient.TextMatrix(flxClient.row, 1)
                txtFundName.Tag = Trim(flxClient.TextMatrix(flxClient.row, 1))
...
Private Sub cmdFundLookUp_Click()
    ...
    Dim adoConn As New ADODB.Connection
    adoConn.Open getConnectionString
    Call LoadManagingAgent(adoConn)
    adoConn.Close
    Frame1.Enabled = False
    Frame2.Enabled = False
    txtSearchClientID.SetFocus
End Sub
Private Sub LoadManagingAgent(conConnection As ADODB.Connection)
   Dim adoRst As New ADODB.Recordset
   Dim SQLStr1 As String
   'SQLStr1 = "SELECT FundID, FundCode, FundName FROM Fund;"
   
   SQLStr1 = "SELECT SupplierID, SupplierName,iif(Type='LLORD','Agent','CLIENT') as STYPE FROM Supplier S " & _
            "where Type in('Agent') ORDER BY TYPE Desc ,SupplierID ;"
   adoRst.Open SQLStr1, conConnection, adOpenStatic, adLockReadOnly
   ...
   flxClient.ColWidth(0) = 100
   flxClient.ColWidth(1) = 1500
   flxClient.ColWidth(2) = 3200
   flxClient.ColWidth(3) = 1300
   ...
   flxClient.ColAlignment(1) = vbLeftJustify
   flxClient.ColAlignment(2) = vbLeftJustify
   lblClientID.Caption = "MA/Client ID"
   lblClientName.Caption = "MA/Client Name"
...
'   End If
   ...
   Dim rRow As Integer
   If adoRst.EOF Then
      MsgBox "No record found.", vbExclamation, "No record found"
   Else
       If sTextBox = "3" Then
            rRow = 1
            flxClient.AddItem ""
               flxClient.TextMatrix(rRow, 0) = "ALL" 'adoRst.Fields.Item("STYPE").Value
               flxClient.TextMatrix(rRow, 1) = "ALL"
                flxClient.RowHeight(rRow) = 280
             rRow = 2
            While Not adoRst.EOF
               ...
               flxClient.RowSel = 1
               flxClient.ColSel = 1
               flxClient.TextMatrix(rRow, 0) = "" 'adoRst.Fields.Item("STYPE").Value
               flxClient.TextMatrix(rRow, 1) = adoRst.Fields.Item("SupplierID").Value
               flxClient.TextMatrix(rRow, 2) = adoRst.Fields.Item("SupplierName").Value
               flxClient.TextMatrix(rRow, 3) = adoRst.Fields.Item("STYPE").Value
...
Private Sub cmdSCYRRPrint_Click()
    ...
    If txtSCYRRStDt.text = "" Then
         MsgBox "Please enter start date", vbInformation, "Warning"
         FocusControl txtSCYRRStDt
         Exit Sub
    End If
...
'   'On Error Resume Next
   adoConn.Open getConnectionString
   If OptInvoiceDate.Value = True Then  'when report is done by invoice date
             adoConn.Execute "UPDATE tblPurInv SET Prn = '';"
            If UCase(txtFundName.text) = "ALL" And UCase(txtClientList.text) = "ALL" Then
                 adoConn.Execute "UPDATE tblPurInv P SET Prn = 'Y' where (isManagementFee=true or P.INV_NO='MF Vebra') AND  " & _
                "P.TRAN_DATE >= #" & Format(txtSCYRRStDt.text, "DD MMMM YYYY") & "#  AND P.TRAN_DATE <= #" & Format(txtSCYRREnDt.text, "DD MMMM YYYY") & "#;"
            End If
            If UCase(txtFundName.text) <> "ALL" And UCase(txtClientList.text) = "ALL" Then
                 adoConn.Execute "UPDATE tblPurInv P SET Prn = 'Y' where (isManagementFee=true or P.INV_NO='MF Vebra')  AND  " & _
                "P.TRAN_DATE >= #" & Format(txtSCYRRStDt.text, "DD MMMM YYYY") & "#  AND P.TRAN_DATE <= #" & Format(txtSCYRREnDt.text, "DD MMMM YYYY") & "# AND " & _
                "SUPP_AC ='" & txtFundName.text & "'"
            End If
            If UCase(txtFundName.text) = "ALL" And UCase(txtClientList.text) <> "ALL" Then
                 adoConn.Execute "UPDATE tblPurInv P SET Prn = 'Y' where  (isManagementFee=true or P.INV_NO='MF Vebra')  AND  " & _
                "P.TRAN_DATE >= #" & Format(txtSCYRRStDt.text, "DD MMMM YYYY") & "#  AND P.TRAN_DATE <= #" & Format(txtSCYRREnDt.text, "DD MMMM YYYY") & "# AND " & _
                "CL_ID ='" & txtClientList.text & "'"
            End If
            If UCase(txtFundName.text) <> "ALL" And UCase(txtClientList.text) <> "ALL" Then
                 adoConn.Execute "UPDATE tblPurInv P SET Prn = 'Y' where  (isManagementFee=true or P.INV_NO='MF Vebra')  AND  " & _
                "P.TRAN_DATE >= #" & Format(txtSCYRRStDt.text, "DD MMMM YYYY") & "#  AND P.TRAN_DATE <= #" & Format(txtSCYRREnDt.text, "DD MMMM YYYY") & "# AND " & _
                "CL_ID ='" & txtClientList.text & "' AND SUPP_AC ='" & txtFundName.text & "'"
            End If
            
             
            
            '  All option selected
                Set Report = reportApp.OpenReport(App.Path & szReportPath & "\ManagingAgentsFeesReport.rpt")
                Report.EnableParameterPrompting = False
                Report.DiscardSavedData
                Report.ParameterFields(1).AddCurrentValue txtClientList.text
                Report.ParameterFields(2).AddCurrentValue txtPropertyName.text
                Report.ParameterFields(3).AddCurrentValue CDate(txtSCYRRStDt.text)
                Report.ParameterFields(4).AddCurrentValue CDate(txtSCYRREnDt.text)
                Report.ParameterFields(5).AddCurrentValue txtClientList.Tag
                Report.ParameterFields(6).AddCurrentValue txtFundName.text
                Report.ParameterFields(7).AddCurrentValue txtPropertyName.Tag
                Load frmReport
                frmReport.LoadReportViewer Report
   Else 'when report is done by invoice date
    'You need to use report table to make this report. Populate report table from payment, allocation and invoice and then just print from report table
    'Note by anol 2022-03-20 PayTransactions -(payment amount field) table has been used because you can use partial payment from a payment line
        If txtClientList.text = "" Then Exit Sub
        Dim rsReprotTable As New ADODB.Recordset
        adoConn.Execute "Delete from ReportManagingAgentFeesPaid"
        If txtClientList.text = "ALL" Then
                ...
                'P is invoice side and p1 is paymentSide
'                adoconn.Execute "Update tblPurInv V ,PayTransactions A,tlbPayment P, tlbPayment P1  " & _
'                                "Set Prn='Y',P1.isPrint=true Where V.My_ID=P.PI AND P.TransactionID=A.ToTran and " & _
'                                "A.FromTran=P1.TransactionID and isManagementFee=true and P.Type in(6,24) AND A.DeleteFlag=false "
                                ' TransDate ,Reference,Description ,PaidAmount,OsAmount,NET,VAT,Total, we need a PI transactionId the
                                'Report table so that you can get ref from payment by linking with allocation
                adoConn.Execute "Update tblPurInv V ,PayTransactions A,tlbPayment P, tlbPayment P1  " & _
                                "Set Prn='Y',P1.isPrint=true Where V.My_ID=P.PI AND P.TransactionID=A.ToTran and " & _
                                "A.FromTran=P1.TransactionID and isManagementFee=true and P.Type =6 AND A.DeleteFlag=false "
                
                'P is invoice side and p1 is PC=fromtran
                adoConn.Execute "Update tblPurInv V ,PayTransactions A,tlbPayment P, tlbPayment P1  " & _
                                "Set Prn='Y',P.isPrint=true Where V.My_ID=P.PI AND P1.TransactionID=A.ToTran and " & _
                                "A.FromTran=P.TransactionID and isManagementFee=true and P1.Type =24  AND A.DeleteFlag=false "
                                
                                
'                 adoconn.Execute "Insert into ReportManagingAgentFeesPaid(ClientID,PropertyID,PI,InvNo,TransDate,TransactionID,reference,Total,PaidAmount,OsAmount ) " & _
'                                 "Select V.CL_ID,V.PropertyID,MY_ID,'PI' & V.slnumber as InvNo,TRAN_DATE,TransactionID,INV_NO,TOTAL_AMOUNT ,0,0 From tblPurInv V,tlbPayment P where P.PI=V.My_ID AND Prn='Y'"
                 adoConn.Execute "Insert into ReportManagingAgentFeesPaid(ClientID,PropertyID,PI,InvNo,TransDate,TransactionID,reference,Total ,PaidAmount,OsAmount) " & _
                                 "Select V.CL_ID,V.PropertyID,MY_ID,'PI' & V.slnumber as InvNo,TRAN_DATE,TransactionID,INV_NO,TOTAL_AMOUNT,0,0  " & _
                                 "From tblPurInv V,tlbPayment P where P.PI=V.My_ID AND Prn='Y' AND V.TransactionType=6 "
                                 
                 adoConn.Execute "Insert into ReportManagingAgentFeesPaid(ClientID,PropertyID,PI,InvNo,TransDate,TransactionID,reference,Total ,PaidAmount,OsAmount) " & _
                                 "Select V.CL_ID,V.PropertyID,MY_ID,'PC' & V.slnumber as InvNo,TRAN_DATE,TransactionID,INV_NO,-TOTAL_AMOUNT,0,0  " & _
                                 "From tblPurInv V,tlbPayment P where P.PI=V.My_ID AND Prn='Y' AND V.TransactionType=7 "
                                 
                 
                 adoConn.Execute "Update tblPurInv V ,tblPurInvSRec A,ReportManagingAgentFeesPaid R  Set R.Description=A.DESCRIPTION " & _
                                 "where A.ParentID=V.MY_ID AND R.PI=V.MY_ID"
                
                szSQL = "SELECT A.ToTran,sum(A.PaymentAmount) as amount1 from tlbPayment P,PayTransactions A  " & _
                                    "where P.Type in (8,9) and A.FromTran=P.TransactionID AND isprint=true AND A.DeleteFlag=false AND  P.PDate >= #" & Format(txtSCYRRStDt.text, "DD MMMM YYYY") & "# AND " & _
                                    "P.PDate <= #" & Format(txtSCYRREnDt.text, "DD MMMM YYYY") & "# group by A.ToTran"
                rsReprotTable.Open szSQL, adoConn, adOpenStatic, adLockReadOnly
                While Not rsReprotTable.EOF
                        rsPayment.Open "Select sum(NET_AMOUNT) as amt, sum(VAT) as VAT1 from tblPurInvSRec S,tlbpayment P where S.ParentID=P.PI AND P.transactionID=" & rsReprotTable("ToTran").Value & "", adoConn, adOpenStatic, adLockReadOnly
                        dblinvNetamount = 0
                        dblinvVATamount = 0
                        ...
                        adoConn.Execute "Update ReportManagingAgentFeesPaid SET PaidAmount=" & rsReprotTable("amount1").Value & ",NET=" & dblinvNetamount & ",VAT=" & dblinvVATamount & "" _
                        & " where TransactionID=" & rsReprotTable("ToTran").Value & ""
                        rsReprotTable.MoveNext
                Wend
                rsReprotTable.Close
                
                szSQL = "SELECT A.FromTran,sum(A.PaymentAmount) as amount1 from tlbPayment P,PayTransactions A, tlbPayment P1  " & _
                                    "where P.Type =24 and  A.TOTran=P.TransactionID and  FromTran=P1.TransactionID  AND P1.isprint=true AND A.DeleteFlag=false  AND P.PDate >= #" & _
                                    Format(txtSCYRRStDt.text, "DD MMMM YYYY") & "# AND P.PDate <= #" & Format(txtSCYRREnDt.text, "DD MMMM YYYY") & "# group by A.FromTran"
                rsReprotTable.Open szSQL, adoConn, adOpenStatic, adLockReadOnly
                While Not rsReprotTable.EOF
                        rsPayment.Open "Select sum(NET_AMOUNT) as amt, sum(VAT) as VAT1 from tblPurInvSRec S,tlbpayment P where S.ParentID=P.PI AND P.transactionID=" & rsReprotTable("FromTran").Value & "", adoConn, adOpenStatic, adLockReadOnly
                        dblinvNetamount = 0
                        dblinvVATamount = 0
                        If Not rsPayment.EOF Then
                               dblinvNetamount = IIf(IsNull(rsPayment("amt").Value), 0, rsPayment("amt").Value)
                        ...
                        rsPayment.Close
                     adoConn.Execute "Update ReportManagingAgentFeesPaid SET PaidAmount=" & -rsReprotTable("amount1").Value & ",NET=" & dblinvNetamount & ",VAT=" & dblinvVATamount & "" _
                     & " where TransactionID=" & rsReprotTable("FromTran").Value & ""
                     rsReprotTable.MoveNext
                Wend
                rsReprotTable.Close
                
                adoConn.Execute "Update ReportManagingAgentFeesPaid set OSAmount=Total-PaidAmount"
                adoConn.Execute "Delete from  ReportManagingAgentFeesPaid where PaidAmount=0"
        Else
               adoConn.Execute "Update tblPurInv Set Prn='N' AND CL_ID ='" & txtClientList.text & "' "
                adoConn.Execute "Update tlbPayment Set isPrint=False AND ClientID ='" & txtClientList.text & "'"
                'for type 6,24
                'P is invoice side and p1 is paymentSide
                adoConn.Execute "Update tblPurInv V ,PayTransactions A,tlbPayment P, tlbPayment P1  " & _
                                "Set Prn='Y',P1.isPrint=true Where V.My_ID=P.PI AND P.TransactionID=A.ToTran and " & _
                                "A.FromTran=P1.TransactionID and isManagementFee=true and P.Type =6 AND CL_ID ='" & txtClientList.text & "' AND A.DeleteFlag=false "
                
                'P is invoice side and p1 is PC=fromtran
                adoConn.Execute "Update tblPurInv V ,PayTransactions A,tlbPayment P, tlbPayment P1  " & _
                                "Set Prn='Y',P.isPrint=true Where V.My_ID=P.PI AND P1.TransactionID=A.ToTran and " & _
                                "A.FromTran=P.TransactionID and isManagementFee=true and P1.Type =24 AND CL_ID ='" & txtClientList.text & "' AND A.DeleteFlag=false "
                                ' TransDate ,Reference,Description ,PaidAmount,OsAmount,NET,VAT,Total, we need a PI transactionId the
                                'Report table so that you can get ref from payment by linking with allocation
                ...
                szSQL = "SELECT A.ToTran,sum(A.PaymentAmount) as amount1 from tlbPayment P,PayTransactions A  " & _
                                    "where P.Type in (8,9) and A.FromTran=P.TransactionID AND isprint=true AND A.DeleteFlag=false AND P.ClientID ='" & txtClientList.text & "' AND P.PDate >= #" & _
                                    Format(txtSCYRRStDt.text, "DD MMMM YYYY") & "# AND P.PDate <= #" & Format(txtSCYRREnDt.text, "DD MMMM YYYY") & "# group by A.ToTran"
                rsReprotTable.Open szSQL, adoConn, adOpenStatic, adLockReadOnly
                While Not rsReprotTable.EOF
                        rsPayment.Open "Select sum(NET_AMOUNT) as amt, sum(VAT) as VAT1 from tblPurInvSRec S,tlbpayment P where S.ParentID=P.PI AND P.transactionID=" & rsReprotTable("ToTran").Value & "", adoConn, adOpenStatic, adLockReadOnly
                        dblinvNetamount = 0
                ...
                szSQL = "SELECT A.FromTran,sum(A.PaymentAmount) as amount1 from tlbPayment P,PayTransactions A, tlbPayment P1  " & _
                                    "where P.Type =24 and  A.TOTran=P.TransactionID and  FromTran=P1.TransactionID  AND P1.isprint=true AND A.DeleteFlag=false AND P.ClientID ='" & txtClientList.text & "' AND P.PDate >= #" & _
                                    Format(txtSCYRRStDt.text, "DD MMMM YYYY") & "# AND P.PDate <= #" & Format(txtSCYRREnDt.text, "DD MMMM YYYY") & "# group by A.FromTran"
                rsReprotTable.Open szSQL, adoConn, adOpenStatic, adLockReadOnly
                While Not rsReprotTable.EOF
                         rsPayment.Open "Select sum(NET_AMOUNT) as amt, sum(VAT) as VAT1 from tblPurInvSRec S,tlbpayment P where S.ParentID=P.PI AND P.transactionID=" & rsReprotTable("FromTran").Value & "", adoConn, adOpenStatic, adLockReadOnly
        ...
        End If
            ...
            '  All option selected
                Set Report = reportApp.OpenReport(App.Path & szReportPath & "\ManagingAgentsFeesPaidReport.rpt")
                Report.EnableParameterPrompting = False
                Report.DiscardSavedData
                Report.ParameterFields(1).AddCurrentValue txtClientList.text
                Report.ParameterFields(2).AddCurrentValue txtPropertyName.text
                ...
                Report.ParameterFields(4).AddCurrentValue CDate(txtSCYRREnDt.text)
                Report.ParameterFields(5).AddCurrentValue txtClientList.Tag
                Report.ParameterFields(6).AddCurrentValue txtPropertyName.Tag
                Load frmReport
                frmReport.LoadReportViewer Report
        'MsgBox "Report has been populated"
   End If
   adoConn.Close
   Set adoConn = Nothing
End Sub
...
'   Me.ZOrder 0
   ...
   OptInvoiceDate.BackColor = MODULEBACKCOLOR
   optPaymentDate.BackColor = MODULEBACKCOLOR
