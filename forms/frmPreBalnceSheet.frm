...
Begin VB.Form frmPreBalnceSheet 
   ...
   EndProperty
   Icon            =   "frmPreBalnceSheet.frx":0000
   LinkTopic       =   "Form2"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ...
   End
End
Attribute VB_Name = "frmPreBalnceSheet"
...
Private Sub cmdPrintNew_Click()
    'Resolved By BOSL. Issue: 0000476
'Modified by Asif. 30-10-2014
...
'
''  All option selected
'   Set Report = reportApp.OpenReport(App.Path & szReportPath & "\BalanceSheetNew.rpt")
'
...
'
'   Report.ParameterFields(2).AddCurrentValue txtSCYRRStDt.text
'   Report.ParameterFields(3).AddCurrentValue txtSCYRREnDt.text
'
...
On Error GoTo CreateReportBalanceSheet

   adoRst.Open "SELECT * FROM ReportBalanceSheet;", adoConn, adOpenStatic, adLockReadOnly
   adoRst.Close

   GoTo LoadBalanceSheetReport

CreateReportBalanceSheet:
   adoConn.Execute _
      "CREATE TABLE ReportBalanceSheet " & _
         "(" & _
...
On Error GoTo ErrorHandler:
    ...
    szSQL = GetBalanceSheetQuery(txtClientList.Tag, txtPropertyName.Tag, txtFundName.Tag, periodFrom, periodTo, reportingDate, sessionID)
    
    adoConn.Execute _
      "DELETE FROM ReportBalanceSheet WHERE SessionID = '" & sessionID & "';"
     'addded by anol 20161025
     adoConn.Execute "DELETE FROM ReportBalanceSheet WHERE ReportingDate < #" & reportingDate & "# ;"
     
    adoConn.Execute _
      "INSERT INTO ReportBalanceSheet " & _
      "(ReportingDate, SessionID, BSTYPE, CLIENTID, HEADER, SUBHEADER, NOMINALCODE, NAME, BALANCE, DEBIT, CREDIT) " & _
        szSQL
...
'
''  All option selected
'   Set Report = reportApp.OpenReport(App.Path & szReportPath & "\BalanceSheet.rpt")
'
'   Report.Database.Tables(1).ConnectionProperties.Item("Database Password") = accessDBPws
'
'   Report.EnableParameterPrompting = False
...
'   Report.ParameterFields(5).AddCurrentValue cboClientID.Column(1)
'   Report.ParameterFields(6).AddCurrentValue txtPropertyName.text
...
On Error GoTo ErrorHandler:
    ...
    szSQL = GetBalanceSheetQuery2(txtClientList.Tag, txtPropertyName.Tag, txtFundName.Tag, periodFrom, periodTo, reportingDate, sessionID)
    ...
    adoConn.Execute _
     ...
     adoConn.Execute _
     "UPDATE ReportBalanceSheet " & _
         "SET " & _
            "Header = 'Capital and Reserves', Balance = Balance * -1 " & _
         "WHERE Header = 'Assets' and SubHeader LIKE '%Liabilities%'; "
         
     adoConn.Execute _
     "UPDATE ReportBalanceSheet " & _
         "SET " & _
            "Header = 'Assets', Balance = Balance * -1 " & _
         "WHERE Header = 'Capital and Reserves' and SubHeader LIKE '%Assets%'; "
     ...
     "UPDATE ReportBalanceSheet " & _
         "SET " & _
            "Header = 'Assets', Balance = Balance * -1 " & _
         "WHERE SubHeader = 'Current Liabilities'; "
...
'  All option selected
   Set Report = reportApp.OpenReport(App.Path & szReportPath & "\BalanceSheetNew2.rpt")
...
'     Call WheelUnHook(flxClient.hWnd)
'    Call WheelHook(flxClient.hWnd)
End Sub
