...
Begin VB.Form frmPreSCExpBudRpt 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Service Charge Budget Report"
   ClientHeight    =   8640
   ClientLeft      =   1125
   ClientTop       =   1935
   ClientWidth     =   7170
   ...
   EndProperty
   Icon            =   "frmPreSCExpBudRpt.frx":0000
   LinkTopic       =   "Form2"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ScaleHeight     =   5963.482
   ScaleMode       =   0  'User
   ScaleWidth      =   6733.004
   ...
   Begin VB.Frame Frame5 
      ...
      Begin VB.OptionButton optSummary 
         ...
         Width           =   1455
      End
      Begin VB.OptionButton optAnalysed 
         Caption         =   "Analysed Report"
         Height          =   375
         Left            =   2025
         TabIndex        =   9
         Top             =   135
   ...
   End
End
Attribute VB_Name = "frmPreSCExpBudRpt"
...
Public LOOKUPCommand As String
...
Private Function ListOfProperties2() As String
   Dim i As Integer

   For i = 1 To flxProperties.Rows - 1
      If flxProperties.TextMatrix(i, 0) = "X" And flxProperties.RowHeight(i) > 0 Then
            If Len(ListOfProperties2) = 0 Then
                ListOfProperties2 = "'" & flxProperties.TextMatrix(i, 1) & "'"
            Else
                ListOfProperties2 = ListOfProperties2 & ", '" & flxProperties.TextMatrix(i, 1) & "'"
           End If
      End If
   Next i
...
End Sub
Private Sub LoadProperty(adoConn As ADODB.Connection) 'loadflxProperties
    Dim szSQL   As String
    Dim r       As Integer
    Dim adoRst As New ADODB.Recordset
...
Private Sub cmdSCYRRPrint_Click()
    ...
    If txtFundName.text = "" Then
         MsgBox "Please select a valid fund", vbInformation, "Fund!!!"
         txtFundName.SetFocus
         Exit Sub
    End If
    ...
    Dim szSQL As String
  If LOOKUPCommand = "" Then 'Only Budget view
        ...
        If optAnalysed.Value Then
           Set Report = reportApp.OpenReport(App.Path & szReportPath & "\SCExpBudRpt_A.rpt")
           Report.Database.Tables(1).ConnectionProperties.Item("Database Password") = accessDBPws
            Report.EnableParameterPrompting = False
            Report.DiscardSavedData
        Else
           Set Report = reportApp.OpenReport(App.Path & szReportPath & "\SCExpBudRpt_S.rpt")
           Report.Database.Tables(1).ConnectionProperties.Item("Database Password") = accessDBPws
            Report.EnableParameterPrompting = False
            Report.DiscardSavedData
         End If
        If Option2.Value = True Then
            'By financial year
            Report.ParameterFields(1).AddCurrentValue CDate(CStr(cmbPeriodFrom.Column(2)))
            Report.ParameterFields(2).AddCurrentValue CDate(cmbPeriodTo.Column(3))
        ...
        Else
             ...
             Report.ParameterFields(3).AddCurrentValue ListOfProperties
        End If
   ElseIf LOOKUPCommand = "A" Then 'Budget versus actual expenditure
        If chkProperty.Value = 0 Then
              If ListOfProperties2 = "" Then
                  MsgBox "Please select a Property", vbInformation, "Property!!!"
                  flxProperties.SetFocus
                  Exit Sub
             End If
        ...
        If IsNull(cmbFinancialYear.Column(0)) Then Exit Sub
        If IsNull(txtClientList.Tag) Then Exit Sub
        If IsNull(txtFundName.Tag) Then Exit Sub
        If Option2.Value = True Then
           periodFrom1 = Format(cmbPeriodFrom.Column(2), "dd mmmm yyyy")
            periodTo = Format(cmbPeriodTo.Column(3), "dd mmmm yyyy")
        Else
            periodFrom1 = CDate(Format(txtSCYRRStDt.text, "dd mmmm yyyy"))
            periodTo = CDate(Format(txtSCYRREnDt.text, "dd mmmm yyyy"))
        End If
        adoConn.Execute "DELETE FROM ReportBudVsAE"
        ' adoConn.Execute "DELETE FROM ReportBudVsAE WHERE ReportingDate < #" & reportingDate & "# ;"
         szSQL = GetBudVsAEQuery(cmbFinancialYear.Column(0), txtClientList.Tag, IIf(chkProperty.Value = 1, "''", ListOfProperties2), txtFundName.Tag, periodFrom1, periodTo, reportingDate, sessionID1, "1")
            adoConn.Execute _
      "INSERT INTO ReportBudVsAE " & _
      "(ReportingDate, SessionID, CLIENTID,NC,tBalance,Budget,Variance) " & _
        szSQL
        
        szSQL = GetBudVsAEQuery(cmbFinancialYear.Column(0), txtClientList.Tag, IIf(chkProperty.Value = 1, "''", ListOfProperties2), txtFundName.Tag, periodFrom1, periodTo, reportingDate, sessionID1, "2")
            adoConn.Execute _
      "INSERT INTO ReportBudVsAE " & _
      "(ReportingDate, SessionID, CLIENTID,NC,tBalance,Budget,Variance) " & _
        szSQL
        ...
        If Option2.Value = True Then 'By financial year
            Report.ParameterFields(1).AddCurrentValue CDate(CStr(cmbPeriodFrom.Column(2)))
            Report.ParameterFields(2).AddCurrentValue CDate(cmbPeriodTo.Column(3))
            Report.ParameterFields(3).AddCurrentValue sessionID1
'        Else
'             Report.ParameterFields(1).AddCurrentValue CDate(Format(txtSCYRRStDt.text, "dd mmmm yyyy"))
'             Report.ParameterFields(2).AddCurrentValue CDate(Format(txtSCYRREnDt.text, "dd mmmm yyyy"))
'             Report.ParameterFields(3).AddCurrentValue ListOfProperties
        End If
    ElseIf LOOKUPCommand = "B" Then 'Budget Profit and loss report
        sessionID1 = GetTimeStamp
        If chkProperty.Value = 0 Then
             If ListOfProperties2 = "" Then
                 MsgBox "Please select a Property", vbInformation, "Property!!!"
        ...
        'adoConn.Execute "DELETE FROM ReportBudVsAE"
        adoConn.Execute "DELETE FROM ReportBudVsAE WHERE ReportingDate < #" & reportingDate & "# ;"
        szSQL = GetBudVsProftandloss(cmbFinancialYear.Column(0), txtClientList.Tag, IIf(chkProperty.Value = 1, "''", ListOfProperties2), txtFundName.Tag, periodFrom1, periodTo, reportingDate, sessionID1, 1)
            adoConn.Execute _
      "INSERT INTO ReportBudVsAE " & _
      "(ReportingDate, SessionID, CLIENTID,NC,tBalance,Budget,Variance) " & _
        szSQL
        szSQL = GetBudVsProftandloss(cmbFinancialYear.Column(0), txtClientList.Tag, IIf(chkProperty.Value = 1, "''", ListOfProperties2), txtFundName.Tag, periodFrom1, periodTo, reportingDate, sessionID1, 2)
            adoConn.Execute _
      "INSERT INTO ReportBudVsAE " & _
      "(ReportingDate, SessionID, CLIENTID,NC,tBalance,Budget,Variance) " & _
        szSQL
        
       szSQL = GetBudVsProftandloss(cmbFinancialYear.Column(0), txtClientList.Tag, IIf(chkProperty.Value = 1, "''", ListOfProperties2), txtFundName.Tag, periodFrom1, periodTo, reportingDate, sessionID1, 3)
            adoConn.Execute _
      "INSERT INTO ReportBudVsAE " & _
      "(ReportingDate, SessionID, CLIENTID,NC,tBalance,Budget,Variance) " & _
        szSQL
'       Dim tsExVal As New ADODB.Recordset
'       Dim curval As Double
'       tsExVal.Open "Select sum(tBalalance) as bal from ReportBudVsAE R,NominalLedger N where  R.NC=R.Code AND sessionID='" & sessionID1 & "')", adoConn, adOpenDynamic, adLockReadOnly
'       If Not tsExVal.EOF Then
'           curval = IIf(IsNull(tsExVal.Fields(0).Value), 0, tsExVal.Fields(0).Value)
'       End If
        Set Report = reportApp.OpenReport(App.Path & szReportPath & "\BudgetPnL.rpt")
        Report.Database.Tables(1).ConnectionProperties.Item("Database Password") = accessDBPws
        Report.EnableParameterPrompting = False
        Report.DiscardSavedData
        If Option2.Value = True Then 'By financial year
            ...
            Report.ParameterFields(2).AddCurrentValue CDate(cmbPeriodTo.Column(3))
            Report.ParameterFields(3).AddCurrentValue sessionID1
            'Report.ParameterFields(3).AddCurrentValue curval
'        Else
'             Report.ParameterFields(1).AddCurrentValue CDate(Format(txtSCYRRStDt.text, "dd mmmm yyyy"))
'             Report.ParameterFields(2).AddCurrentValue CDate(Format(txtSCYRREnDt.text, "dd mmmm yyyy"))
'             Report.ParameterFields(3).AddCurrentValue ListOfProperties
   ...
   If LOOKUPCommand = "A" Then 'Budget versus actual expenditure
        ' adoConn.Execute "DELETE FROM ReportBudVsAE WHERE SessionID = '" & sessionID1 & "';"
         adoConn.Close
         Set adoConn = Nothing
   End If
...
Private Sub Form_Load()
   ...
   optAnalysed.BackColor = MODULEBACKCOLOR
   optSummary.BackColor = MODULEBACKCOLOR
