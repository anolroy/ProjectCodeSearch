...
Begin VB.Form frmPreProfit_n_Loss 
   ...
   EndProperty
   Icon            =   "frmPreProfit_n_Loss.frx":0000
   LinkTopic       =   "Form2"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ...
   End
End
Attribute VB_Name = "frmPreProfit_n_Loss"
...
'
''  All option selected
'   Set Report = reportApp.OpenReport(App.Path & szReportPath & "\PnL_Report.rpt")
'
...
On Error GoTo CreateReportProfitAndLoss

   adoRst.Open "SELECT * FROM ReportProfitAndLoss;", adoConn, adOpenStatic, adLockReadOnly
   adoRst.Close

   GoTo LoadProfitAndLossReport

CreateReportProfitAndLoss:
   adoConn.Execute _
      "CREATE TABLE ReportProfitAndLoss " & _
         "(" & _
...
On Error GoTo ErrorHandler:
    ...
    szSQL = GetProfitAndLossQuery(txtClientList.Tag, txtPropertyName.Tag, txtFundName.Tag, periodFrom, periodTo, reportingDate, sessionID)
    
    adoConn.Execute _
      "DELETE FROM ReportProfitAndLoss WHERE SessionID = '" & sessionID & "';"
      'added by anol 20161025
 adoConn.Execute "DELETE FROM ReportProfitAndLoss WHERE ReportingDate < #" & reportingDate & "# ;"
 
    adoConn.Execute _
      "INSERT INTO ReportProfitAndLoss " & _
      "(ReportingDate, SessionID, CLIENTID, HEADER, SUBHEADER, NOMINALCODE, NAME, BALANCE, DEBIT, CREDIT) " & _
        szSQL
...
'  All option selected
   Set Report = reportApp.OpenReport(App.Path & szReportPath & "\ProfitAndLoss.rpt")
