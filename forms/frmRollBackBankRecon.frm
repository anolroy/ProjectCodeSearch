...
Begin VB.Form frmRollBackBankRecon 
   ...
   Begin VB.Frame fraCheckSumPayment 
      ...
      Begin VB.CommandButton cmdFixWestbourne 
         ...
         Height          =   525
         Left            =   4455
         Picture         =   "frmRollBackBankRecon.frx":0000
         TabIndex        =   66
      ...
      Begin VB.CommandButton cmdfixEvron 
         ...
         Height          =   525
         Left            =   4410
         Picture         =   "frmRollBackBankRecon.frx":5D2FA
         TabIndex        =   65
         ToolTipText     =   "Click to fix the Data"
         Top             =   2700
         Visible         =   0   'False
      ...
      Begin VB.CommandButton cmdFixPayAllocation 
         ...
         Height          =   435
         Left            =   4455
         Picture         =   "frmRollBackBankRecon.frx":BA5F4
         TabIndex        =   60
         ToolTipText     =   "Click to fix the Data"
         Top             =   1485
         Visible         =   0   'False
   ...
   Begin VB.Frame fraReport 
      ...
      Begin VB.CommandButton cmdShowReport 
         ...
         Height          =   435
         Left            =   5760
         Picture         =   "frmRollBackBankRecon.frx":1178EE
         TabIndex        =   47
         ToolTipText     =   "Click to fix the Data"
         Top             =   3555
         Width           =   1590
      ...
      Begin VB.CommandButton cmdFixTransactions 
         ...
         Height          =   435
         Left            =   3960
         Picture         =   "frmRollBackBankRecon.frx":174BE8
         TabIndex        =   46
         ToolTipText     =   "Click to fix the Data"
         Top             =   3555
         Width           =   1725
   ...
   Begin VB.Frame fraVatamount 
      ...
      Begin VB.CommandButton cmdFixVat 
         ...
         Height          =   435
         Left            =   6030
         Picture         =   "frmRollBackBankRecon.frx":1D1EE2
         TabIndex        =   40
         ToolTipText     =   "Click to fix the Data"
         Top             =   3060
         Width           =   1725
   ...
   Begin VB.Frame fraManualDemand 
      ...
      Begin VB.CommandButton cmdConvert 
         ...
         Height          =   435
         Left            =   6120
         Picture         =   "frmRollBackBankRecon.frx":22F1DC
         TabIndex        =   38
         ToolTipText     =   "Click to fix the Data"
         Top             =   3330
         Width           =   1725
   ...
   Begin VB.Frame FraReconciliation 
      ...
      Begin VB.CommandButton cmdRollbackBreconciliation 
         ...
         Height          =   570
         Left            =   3195
         Picture         =   "frmRollBackBankRecon.frx":28C4D6
         TabIndex        =   37
         ToolTipText     =   "Click to fix the Data"
         Top             =   3195
         Width           =   3705
   ...
   End
End
Attribute VB_Name = "frmRollBackBankRecon"
...
Private Sub cmdFixPayAllocation_Click()
   If MsgBox("Paytransactions JGC(PI6390) will be fixed if you run this procedure. are you sure?", vbYesNo, "Confirm?") = vbYes Then
        Dim adoConn As New ADODB.Connection
...
Private Function LastBankReconDate(adoConn As ADODB.Connection) As String
    ...
    Dim szSQL As String
    Dim temp As String
    szSQL = "Select MAX(StatementDate) as DT FROM tlbBankReconClosingBal B where ClientID='" & txtClientList.Tag & "' AND BankCode='" & txtBC.Tag & "'"
...
'this procedure is tesing before rollback , selected period is consistence then it will start rollback.
'The first recondate of tlbBankReconClosingBal openning balance is not currect even acc balance is not currect
'written by anol 2019 05 28
    ...
    Else
        ...
        MsgBox "Failed to rollback bank reconciliation", vbInformation, "Check sum failed"
    End If
End Function
Private Sub cmdRollbackBreconciliation_Click()
    ...
    'Written by anoll 21 July 2016
        ...
        If MsgBox("Do you wish to rollback all reconciled bank reconciliations back to and including the following date : " & cboCurStDt.Value & " ? ", vbYesNo, "Warning") = vbYes Then
           ...
           ' If checksumRollbankBankrecon(adoConn) = True And Format(LastBankReconDate(adoConn), "dd MMM yyyy") = Format(cboCurStDt.Value, "dd MMM yyyy") Then  ' we are only checking the last dated bank reconciliation if that is consistent
            adoRst.Open "Select *  FROM tlbBankReconClosingBal where StatementDate>=#" & Format(cboCurStDt.Value, "dd MMM yyyy") & _
                    "# AND ClientID='" & txtClientList.Tag & "' AND BankCode='" & txtBC.Tag & "' order by StatementDate ", adoConn, adOpenKeyset, adLockReadOnly
            ReDim ReConDates(adoRst.RecordCount) As String
            While Not adoRst.EOF
                ReConDates(i) = Format(adoRst.Fields("StatementDate").Value, "dd MMM yyyy")
                i = i + 1
                adoRst.MoveNext
            Wend
            adoRst.Close
            Set adoRst = Nothing
            For j = 0 To UBound(ReConDates) - 1
                adoConn1.Open getConnectionString
                adoConn1.BeginTrans
                If checksumRollbankBankrecon(adoConn1, ReConDates(j)) = True Then
                    If RollbankBankreconForAdate(adoConn1, ReConDates(j)) = True Then
                        adoConn1.CommitTrans
                        'MsgBox "Routine completed successfully.", vbInformation, "Bank Reconciliation Rollback"
                        Debug.Print "Run : " & ReConDates(j)
                    Else
                        adoConn1.RollbackTrans
                        'MsgBox "Could not run the routine.", vbInformation, "Please check data is consistent"
                        Debug.Print 2
                        adoConn1.Close
               ...
               Else
                    ...
                    MsgBox "Could not run the routine.", vbInformation, "Please check data is consistent"
                    cmdRollbackBreconciliation.Enabled = True
                    Debug.Print "Could not run the routine." & ReConDates(j)
                    GoTo XX
               End If
               adoConn1.Close
               Set adoConn1 = Nothing
            ...
            adoConn.Close
            Set adoConn = Nothing
            'data comes lsower so need to close and make connection again
            adoConn.Open getConnectionString
            Call loadcboCurStDt(adoConn)
            If cboCurStDt.ListCount > 0 Then
                cboCurStDt.ListIndex = 0
            End If
        End If
XX:
     adoConn.Close
...
End Sub
Private Function RollbankBankreconForAdate(adoConn As ADODB.Connection, szStatementDate As String) As Boolean
On Error GoTo Err
        Dim adoRst As New ADODB.Recordset
        adoConn.Execute "DELETE FROM tlbBankReconClosingBal where ClientID='" & txtClientList.Tag & "' AND BankCode='" & txtBC.Tag & "' AND StatementDate=#" & Format(szStatementDate, "dd MMM yyyy") & "#"
        adoConn.Execute "DELETE FROM tlbBankReconcilation where ClientID='" & txtClientList.Tag & "' AND BankCode='" & txtBC.Tag & "' AND ReconDate=#" & Format(szStatementDate, "dd MMM yyyy") & "# "

            
        'Modified by anol 20170117 some of the reconciliation where clinet ID was null  was not updating.
        adoConn.Execute "Update tlbReceipt,Units,Property SET tlbReceipt.ReconNow= NULL,tlbReceipt.Reconciled= NULL where " & _
                "tlbReceipt.UnitID=Units.UnitNumber AND Units.PropertyID=Property.PropertyID and Property.ClientID='" & txtClientList.Tag & "' AND BankCode='" & txtBC.Tag & _
                "' AND CDate( Left(iif(isnull(Reconnow),#31 Mar 1930#,Reconnow),10))=#" & Format(szStatementDate, "dd MMM yyyy") & "# AND Reconnow is not NULL"
        ...
        Else
              ...
              adoRst.Fields("ProjClbal").Value & ",PCB=0,ClosingBal=" & adoRst.Fields("ProjClbal").Value & " where CLIENT_ID='" & txtClientList.Tag & "' AND NominalCode='" & txtBC.Tag & "'"
              ',PCB=" & adoRst.Fields("ProjClbal").Value & "
        End If
        RollbankBankreconForAdate = True
        Exit Function
Err:
        RollbankBankreconForAdate = False
End Function
'Private Function checksumRollbankBankreconOthersMaster(adoconn As ADODB.Connection) As Boolean ' true means cheack passed and false means failed
'     Dim szSQL As String
'     Dim adorst As New ADODB.Recordset
'     Dim cValue As Boolean
'
...
'   adorst.Open szSQL, adoconn, adOpenStatic, adLockReadOnly
'   While Not adorst.EOF
'        cValue = checksumRollbankBankreconOthers(adoconn, adorst("StatementDate").Value)
...
'  Exit Function
'XX:
'    checksumRollbankBankreconOthersMaster = False
'End Function
'Private Function checksumRollbankBankreconOthers(adoconn As ADODB.Connection, szStatementDate As String) As Boolean ' true means cheack passed and false means failed
''this procedure is tesing before rollback , selected period is consistence then it will start rollback.
''on second
''on the first recondate of tlbBankReconClosingBal openning balance is not currect even acc balance is not currect
'    Dim adorst As New ADODB.Recordset
'    Dim dbamount As Double
'    Dim dbamountCompr As Double
'    Dim szSQL As String
'    szSQL = "Select (ProjClBal-StOpenBal) as amt FROM tlbBankReconClosingBal B where ClientID='" & txtClientList.Tag & "' AND BankCode='" & txtBC.Tag & _
'            "' AND StatementDate=#" & Format(szStatementDate, "dd MMM yyyy") & "#"
'    adorst.Open szSQL, adoconn, adOpenStatic, adLockReadOnly
'    If Not adorst.EOF Then
'        dbamountCompr = IIf(IsNull(adorst("amt").Value), "0", adorst("amt").Value)
...
'    Set adorst = Nothing
'    If dbamountCompr - dbamount = 0 Then
'        checksumRollbankBankreconOthers = True
'    Else
'        checksumRollbankBankreconOthers = False
'    End If
'End Function
...
Private Sub flxIssueList_Click()
    ...
    If iSelRow = 4 Then
     ...
     If iSelRow = 8 Then
         ...
         Call ChecksumValidationOnPayAllocation
...
'     End If
     ...
     If iSelRow = 11 Then 'Batch Receipts not Bank Reconciling issue 764 date 2019/05/10
        If MsgBox("Do you want to run the routine to fix 'Batch Receipts not Bank Reconciling issue'?", vbYesNo) = vbNo Then Exit Sub
...
''    cboIssue.ListIndex = 0
    ...
    Dim adoConn As New ADODB.Connection
    adoConn.Open getConnectionString
    Call LoadFirstBankCode(adoConn)
    Call loadcboCurStDt(adoConn)
    adoConn.Close
   '#
   Call WheelHook(Me.hWnd)
End Sub
Private Function LoadFirstBankCode(adoConn As ADODB.Connection)
    Dim szSQL As String
    Dim adoRst As New ADODB.Recordset
   ...
   adoRst.Close
End Function
Private Function loadcboCurStDt(adoConn As ADODB.Connection)
    Dim szSQL As String
    Dim adoRst As New ADODB.Recordset
     Dim ReConDates()        As String
