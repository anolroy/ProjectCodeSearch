...
Begin VB.Form frmNLAnalysis 
   ...
   EndProperty
   Icon            =   "frmNLAnalysis.frx":0000
   LinkTopic       =   "Form2"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ScaleHeight     =   3903.182
   ScaleMode       =   0  'User
   ScaleWidth      =   6394.945
   ...
   Begin VB.Frame Frame6 
      ...
      Top             =   4365
      Width           =   6585
      Begin VB.CheckBox chkPostingDate 
         Caption         =   "Report By Posting Date"
         Height          =   195
         Left            =   2025
         TabIndex        =   14
         Top             =   225
   ...
   End
End
Attribute VB_Name = "frmNLAnalysis"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'Name : nominal history report
Option Explicit
...
'   flxClient.Width = 5175
   ...
   adoConn.Open getConnectionString
        ...
        szSQL = "SELECT * From NominalLedger where ClientID='" & txtClientList.Tag & "' ORDER BY Code;"
...
'           flxClient.AddItem ""
        ...
        While Not rstRec.EOF
           ...
           flxClient.ColSel = 1
           flxClient.TextMatrix(rRow, 0) = ""
           flxClient.TextMatrix(rRow, 1) = IIf(rstRec.Fields.Item(0) = Null, "", rstRec.Fields.Item(0))
           flxClient.TextMatrix(rRow, 2) = IIf(rstRec.Fields.Item(1) = Null, "", rstRec.Fields.Item(1))
           flxClient.RowHeight(rRow) = 280
           rstRec.MoveNext
...
'    fmePropertyLookup.Top = txtFundName.Top
...
'   gridPropertyLookup.ZOrder 0
'   gridPropertyLookup.Refresh
'
        ...
        End If
         If sTextBox = "4" Then
            ...
            If LOOKUPCommand = "NLSt" Then
                 txtNLStart.text = flxClient.TextMatrix(flxClient.row, 1)
            End If
            If LOOKUPCommand = "NLEnd" Then
                txtNLEnd.text = flxClient.TextMatrix(flxClient.row, 1)
            End If
...
Private Sub txtNLEnd_GotFocus()
    SelTxtInCtrl txtNLEnd
End Sub

Private Sub txtNLEnd_KeyPress(KeyAscii As MSForms.ReturnInteger)
    If KeyAscii = 13 Then
        Option2.SetFocus
    End If
    If KeyAscii = 27 Then
   ...
   If KeyAscii = 46 Then
      If InStr(1, txtNLEnd.text, ".") > 0 Then
         KeyAscii = 0
      End If
   End If
End Sub

Private Sub txtNLStart_GotFocus()
    SelTxtInCtrl txtNLStart
End Sub

Private Sub txtNLStart_KeyPress(KeyAscii As MSForms.ReturnInteger)
   ' DigitTextKeyPress txtNLStart, KeyAscii, False doesn't work here
    If KeyAscii = 13 Then
        txtNLEnd.SetFocus
    End If
   ...
   If KeyAscii = 46 Then
      If InStr(1, txtNLStart.text, ".") > 0 Then
         KeyAscii = 0
      End If
   End If
...
Private Sub cmdSCYRRPrint_Click()
   ...
   If txtNLStart.text = "" Then
      ShowMsgInTaskBar "Please enter nominal code (start).", , "N"
      FocusControl txtNLStart
      Exit Sub
   End If
   If txtNLEnd.text = "" Then
      ShowMsgInTaskBar "Please enter nominal code (End).", , "N"
      FocusControl txtNLEnd
      Exit Sub
   End If
...
'   Set adoConn = Nothing
   ...
   If LOOKUPparam = "NLListing" Then
         If Not CBool(chkPostingDate.Value) Then
            Set Report = reportApp.OpenReport(App.Path & szReportPath & "\NLListingReport.rpt")
         Else
            Set Report = reportApp.OpenReport(App.Path & szReportPath & "\NLListingReportP.rpt")
         End If
...
'  All option selected
    If LOOKUPparam = "" Then
        If Not CBool(chkPostingDate.Value) Then
            Set Report = reportApp.OpenReport(App.Path & szReportPath & "\nominalAnalysisNew1.rpt")
        Else
            Set Report = reportApp.OpenReport(App.Path & szReportPath & "\nominalAnalysisNew1P.rpt")
        End If
        
   End If
   strSelectionFormula = "{NLPosting.NOMINAL_CODE} in '" & txtNLStart.text & "' to '" & txtNLEnd.text & "'"
   If txtClientList.Tag <> "ALL" Then
        strSelectionFormula = strSelectionFormula & " AND {NLPOSTING.CLIENTID} = '" & txtClientList.Tag & "'"
   End If
   If txtPropertyName.Tag <> "ALL" Then
        strSelectionFormula = strSelectionFormula & " AND {NLPOSTING.PROPERTY_ID} = '" & txtPropertyName.Tag & "'"
   End If
   If txtFundName.Tag <> "ALL" Then
        strSelectionFormula = strSelectionFormula & " AND {NLPOSTING.FUND_ID} = " & txtFundName.Tag & ""
   End If
   strSelectionFormula = strSelectionFormula & " AND {NLPosting.DeleteFlag}= false"
   
   'For Nominal history report transaction date is  managed by report selection formula or formula
   'So we don't have to pass it from here anol 20180505
   'For report by Posted date I should make a new report
   'so remming this part
'''   If Option1.Value = True Then 'By date range
'''        If CBool(chkPostingDate.Value) Then
'''            strSelectionFormula = strSelectionFormula & " AND {NLPosting.POSTED_DATE} >=#" & CDate(txtSCYRRStDt.text) & "# AND {NLPosting.POSTED_DATE}<=#" & CDate(txtSCYRREnDt.text) & "#"
'''        Else
'''            strSelectionFormula = strSelectionFormula & " AND {NLPosting.TRANSACTION_DATE} >=#" & CDate(txtSCYRRStDt.text) & "# AND {NLPosting.TRANSACTION_DATE}<=#" & CDate(txtSCYRREnDt.text) & "#"
'''        End If
'''
'''   End If
'''
'''    If Option2.Value = True Then 'by financial Year
'''        If CBool(chkPostingDate.Value) Then
'''            strSelectionFormula = strSelectionFormula & " AND {NLPosting.POSTED_DATE} >=#" & CDate(CStr(cmbPeriodFrom.Column(2))) & "# AND {NLPosting.POSTED_DATE}<=#" & CDate(cmbPeriodTo.Column(3)) & "#"
'''        Else
'''            strSelectionFormula = strSelectionFormula & " AND {NLPosting.TRANSACTION_DATE} >=#" & CDate(CStr(cmbPeriodFrom.Column(2))) & "# AND {NLPosting.TRANSACTION_DATE}<=#" & CDate(cmbPeriodTo.Column(3)) & "#"
'''        End If
'''    End If
   'strSelectionFormula = strSelectionFormula & "{tblpurinv.dueDate} >=#" & CDate(txtSCYRRStDt.text) & "# AND {tblpurinv.dueDate}<=#" & CDate(txtSCYRREnDt.text) & "#"
   'strSelectionFormula = strSelectionFormula & "{tblpurinv.dueDate} >=#" & CDate(txtSCYRRStDt.text) & "# AND {tblpurinv.dueDate}<=#" & CDate(txtSCYRREnDt.text) & "#"
   ...
   Report.ParameterFields(3).AddCurrentValue IIf(txtFundName.Tag = "ALL", 0, Val(txtFundName.Tag))
   Report.ParameterFields(4).AddCurrentValue txtNLStart.text
   Report.ParameterFields(5).AddCurrentValue txtNLEnd.text
'Debug.Print CDate(Format(txtSCYRREnDt.text, "MM/DD/YYYY"))
    If Option2.Value = True Then 'by financial Year
        Report.ParameterFields(6).AddCurrentValue CDate(CStr(cmbPeriodFrom.Column(2)))
        Report.ParameterFields(7).AddCurrentValue CDate(cmbPeriodTo.Column(3))
        Report.ParameterFields(11).AddCurrentValue "Financial period : " & cmbFinancialYear.text & vbNewLine & vbNewLine & cmbPeriodFrom.text & " - " & cmbPeriodTo.text
    End If
   If Option1.Value = True Then 'By date range
        Report.ParameterFields(6).AddCurrentValue CDate(Format(txtSCYRRStDt.text, "DD MMMM YYYY"))
        Report.ParameterFields(7).AddCurrentValue CDate(Format(txtSCYRREnDt.text, "DD MMMM YYYY"))
        Report.ParameterFields(11).AddCurrentValue "Date Range :  " & CDate(Format(txtSCYRRStDt.text, "DD MMMM YYYY")) & " - " & CDate(Format(txtSCYRREnDt.text, "DD MMMM YYYY"))
   End If
...
Err:
   If LOOKUPparam = "" Then
        MsgBox "The Report file 'NominalAnalysisNew1P.rpt' is missing. Please contact PCM Consulting Support. ", vbInformation, "Warning!"
   End If
    If LOOKUPparam = "NLListing" Then
        MsgBox "Report file (NLListingReportP.rpt) is missing. Please contact PCM Consulting Support. ", vbInformation, "Warning!"
      End If
End Sub
...
Private Sub Form_Load()
   ...
   chkPostingDate.BackColor = MODULEBACKCOLOR
...
Private Sub txtSearchProperty_GotFocus()
    gridPropertyLookup.Refresh
End Sub
