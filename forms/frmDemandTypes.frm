...
'Dim clsDemandType1 As clsDemandType
'Public objDemandType As New colDemandType
...
'   adoRst!NominalNameforTotal = txt3.text
   ...
   If txt1.text = "" Then
        adoRst!NominalNameforAmount = " " 'X.text
   Else
         adoRst!NominalNameforAmount = txt1.text 'X.text
   End If
   adoRst.Update
   adoRst.Close
   Set adoRst = Nothing
...
'
   ...
   szSQL = "SELECT ID, Type, Prefix, NominalCodeforAmount, InvCrd, " & _
               ...
               "NominalCodeforTotal, NominalNameforTotal, TransactionType, " & _
               "CategoryCode, PaymentDates, DTGroup, DemandReportName, " & _
               "Spare1, PropertyID, EmailInvoiceTemplate, StatementTemplate,Consolidated,LastModifiedBy,LastModifiedDate,CreatedBy,CreatedDate  " & _
            "FROM DemandTypes"
   adoRst.Open szSQL, adoConn, adOpenDynamic, adLockOptimistic
...
& "SecondaryCode AS S,Property  WHERE Property.ClientID=T.ClientID AND T.CAType = S.Code AND S.PrimaryCode = 'CAT' AND Property.PropertyID = '" & txtProperty.Tag & "' ORDER By t.CADisOrder"
   ...
   Dim x3 As String
   Dim x4 As String
   'We are doing control accounts has been filled or not here validation
   While Not rsCheck.EOF
        If rsCheck("CAName").Value = "Sales Ledger Control" Then
            x1 = rsCheck("NCODE").Value
            x2 = rsCheck("NName").Value
   ...
   If x1 = "" Then
                    ...
                    adoConn.Close
                    Set adoConn = Nothing
                    MsgBox "Please setup a Sales Ledger Control Account in Tools/Configuration/Control Account", vbInformation, "Warning"
                    Exit Sub
            End If
             If x3 = "" Then
                        rsCheck.Close
                        ...
                        adoConn.Close
                        Set adoConn = Nothing
                        MsgBox "Please setup a Output VAT Control Account in Tools/Configuration/Control Account", vbInformation, "Warning"
                        Exit Sub
                    End If
'  ---------------------------------------------------
'  'InvCrd' field is required in the table. This field is no longer in use.
...
'     cmdCopy.Enabled = True
     fraCommands.Enabled = True
End Sub
Private Sub PropertyControlAcoountWarning()
   Dim adoConn As New ADODB.Connection
   Dim rsCheck As New ADODB.Recordset
   Dim szSQL As String
   adoConn.Open getConnectionString
...
Private Sub flxClient_Click()
        ...
        If strCommandSource = "1" Then
                ...
                'written by anol 20210123
                Call PropertyControlAcoountWarning
...
'Debug.Print szSQL
   ...
   End If
   flxDemandTypes.Rows = RecordCount(adoRst) + 1
   Dim icounter As Integer
''   icounter = objDemandType.Count
''
'''    Call objDemandType.Add("Anol", "1", "1", "1", "1", "1", "1", "1", clsDemandType1)
'''    Call objDemandType.Add("Neil", "1", "1", "1", "1", "1", "1", "1", clsDemandType1)
'''    Call objDemandType.Add("Sum", "1", "1", "1", "1", "1", "1", "1", clsDemandType1)
'''
'''    MsgBox objDemandType(3).ClientID
'''      Exit Sub
''
''   With adoRst.Fields
''        For i = 0 To adoRst.RecordCount - 1
''                Call objDemandType.Add(IIf(IsNull(.Item("ClientID")), "", .Item("ClientID")), _
''                    IIf(IsNull(.Item("ClientName")), "", .Item("ClientName")), _
''                    IIf(IsNull(.Item("PropertyID")), "", .Item("PropertyID")), _
''                    IIf(IsNull(.Item("PropertyName")), "", .Item("PropertyName")), _
''                    IIf(IsNull(.Item("Type")), "", .Item("Type")), _
''                    IIf(IsNull(.Item("ID")), "", .Item("ID")), _
''                    IIf(IsNull(.Item("ClientName")), "", .Item("ClientName")), _
''                    IIf(IsNull(.Item("PropertyName")), "", .Item("PropertyName")), objDemandType)
''                    adoRst.MoveNext
''        Next
''  End With
''  Exit Sub
''  Call objDemandType.formatASTree
''  ' I tried to load all data into a Class then try to  build a tree like view. But that is taking even more time
''  'so datagrid vs class: result datagrid is faster
''  For i = 1 To objDemandType.Count - 1
''         flxDemandTypes.TextMatrix(i, 1) = objDemandType(i).ClientID
''         flxDemandTypes.TextMatrix(i, 2) = objDemandType(i).ClientName
''         flxDemandTypes.TextMatrix(i, 3) = objDemandType(i).PropertyID
''         flxDemandTypes.TextMatrix(i, 4) = objDemandType(i).PropertyName
''         flxDemandTypes.TextMatrix(i, 5) = objDemandType(i).DemandType
''         flxDemandTypes.TextMatrix(i, 6) = objDemandType(i).ID
''         flxDemandTypes.TextMatrix(i, 7) = objDemandType(i).ClientName1
''         flxDemandTypes.TextMatrix(i, 8) = objDemandType(i).PropertyName1
''  Next i
''
''
''  Exit Sub
   ...
   'Now the problem is is it takes time to read from the grid when row is greater than 1500, so you need to update the grid while loading at first time. Time to load optimization  failed
                Dim a As Integer
                Dim b As Integer
                ...
                If flxDemandTypes.Rows > 500 Then Exit Sub  'this is the hacking that will save loading time
