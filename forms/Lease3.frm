...
Begin VB.Form frmLease3 
   ...
   Begin TabDlg.SSTab tabLease 
      ...
      TabPicture(0)   =   "Lease3.frx":08CA
      Tab(0).ControlEnabled=   0   'False
      Tab(0).Control(0)=   "Frame2"
      Tab(0).ControlCount=   1
      TabCaption(1)   =   "&Rent Charges"
      TabPicture(1)   =   "Lease3.frx":08E6
      ...
      Tab(2).Control(5)=   "cmdSaveRentAnalysis"
      ...
      Begin VB.Frame Frame1 
         ...
         Begin VB.ComboBox cboFreqSC 
            Height          =   315
            ItemData        =   "Lease3.frx":0A18
            Left            =   1470
            List            =   "Lease3.frx":0A1A
            TabIndex        =   49
            Top             =   1204
            Width           =   2715
         End
      ...
      Begin VB.Frame Frame2 
         ...
         Begin VB.ComboBox cboType 
            Height          =   315
            ItemData        =   "Lease3.frx":0A1C
            Left            =   1800
            List            =   "Lease3.frx":0A1E
            TabIndex        =   11
            Top             =   2040
            Width           =   1995
         End
...
Dim RENT_REVIEW_ADDNEW_EDIT As Boolean
...
Private Sub cboFreqBR_Click()
   If txtRentStartDate.text <> "" Then RentNextDueDate
End Sub
...
Private Sub cboFreqSC_Click()
   If txtPayableFrom.text <> "" And cboSCPayable.text = "Yes" Then SetNextDueDtSC
End Sub
...
Private Sub cboInsurancePayable_Click()
...
Private Sub cboSCPayable_Click()
...
Private Sub cmdCancelEdit_Click()
   If cmdSaveRentCrg.Enabled Then
      MsgBox "Please save/cancel the Rent Charge editing first.", vbCritical + vbOKOnly, "Rent Charge"
      Exit Sub
   End If
   Call EmptyBoxes
   Call GetTenantsWithLease
   Call DisableBoxes
   ConfigureFlxRentCharges
   RentChargesControlsMode DefaultMode
End Sub
...
'   txtNextDueDate.text = ""
   If MsgBox("Are you sure to cancel Rent Charges changes?", vbQuestion + vbYesNo, "Cancel") = vbYes Then
      RentChargesControlsMode DefaultMode
      RENTCHARGES_EDIT = 0
   End If
End Sub
...
Private Sub cmdEditRentCrg_Click()
   If MsgBox("Do you want to edit the charge?", vbQuestion + vbYesNo, "Edit") = vbYes Then
      RentChargesControlsMode EditMode
...
Private Sub cmdNewRentCrg_Click()
   If MsgBox("Do you want to add new charge?", vbQuestion + vbYesNo, "Add New") = vbYes Then
      RentChargesControlsMode NewEntryMode
      RENTCHARGES_EDIT = 0
   End If
End Sub
...
Private Sub cmdSaveRentAnalysis_Click()
   ...
   If Not RENT_REVIEW_ADDNEW_EDIT Then
      lID = FindXID
...
Private Function SaveUpdateLease(bSaveUpdate As Boolean) As Boolean
   ...
   If cboSCPayable.text = "Yes" Then
      ...
      End If
       ...
       If cboFreqSC.text = "" Then
           ...
           Exit Function
       End If
       If txtSCPercentage.text = "" And txtPPSqFoot.text = "" And txtAnnualService.text = "" And txtGlobalAmount.text = "" Then
           MsgBox "You must set a Charging method!", vbOKOnly + vbCritical, "Frequency Required"
           SaveUpdateLease = False
           Exit Function
       End If
   End If
   ...
   If flxRentCharges.TextMatrix(1, 0) <> "" Then
      ...
      SQLStr2 = "SELECT * " & _
                "FROM LRentCharges;"
      Set Rst2 = Conn1.OpenResultset(SQLStr2, rdOpenDynamic, rdConcurRowVer)
      Dim iRow As Integer

      For iRow = 1 To flxRentCharges.Rows - 1
         Rst2.AddNew

         Rst2!RentCharges = flxRentCharges.TextMatrix(iRow, 9)
         Rst2!LeaseID = txtLeaseID.text
         Rst2!RentChargeDept = flxRentCharges.TextMatrix(iRow, 2)
         Rst2!BRfrequency = flxRentCharges.TextMatrix(iRow, 4)
         Rst2!BRStartDate = CDate(Format(flxRentCharges.TextMatrix(iRow, 5), "dd mmmm yyyy"))
         Rst2!BRNextDueDate = CDate(Format(flxRentCharges.TextMatrix(iRow, 7), "dd mmmm yyyy"))
         Rst2!BRTotal = flxRentCharges.TextMatrix(iRow, 6)
         Rst2!BRAmount = flxRentCharges.TextMatrix(iRow, 8)
   ...
   If cboSCPayable.text = "Yes" Then
      ...
      Rst1!ServiceChargeDept = cboServiceChargeDept.Column(0)
   Else
      Rst1!SCPayable = "N"
   End If
...
'   ****************************************************
   If cboInsurancePayable.text = "Yes" Then
      ...
      Rst1!InsuranceStartDate = CDate(Format(txtInsuranceStartDate.text, "dd mmmm yyyy"))
      Rst1!InsuranceEndDate = CDate(Format(txtInsuranceEndDate.text, "dd mmmm yyyy"))
      Rst1!InsuranceDemandType = CByte(IIf(cboInsuranceDemandType.text <> "", cboInsuranceDemandType.ListIndex + 1, 0))
      If txtInsuranceNextDueDate.text <> "" Then Rst1!InsuranceNextDueDate = CDate(Format(txtInsuranceNextDueDate.text, "dd mmmm yyyy"))
      ...
      Rst1!InsuranceDept = cboInsuranceDept.Column(0)
   Else
...
'      ' Add a row at the bottom of the grid
      If RENTCHARGES_EDIT = 0 Then
         ...
         flxRentCharges.TextMatrix(flxRentCharges.Rows - 1, 0) = CByte(cboBRDemandType.ListIndex + 1)
         flxRentCharges.TextMatrix(flxRentCharges.Rows - 1, 1) = cboBRDemandType.text
         flxRentCharges.TextMatrix(flxRentCharges.Rows - 1, 2) = cboRentChargeDept.Column(0)
         flxRentCharges.TextMatrix(flxRentCharges.Rows - 1, 3) = cboRentChargeDept.text
         flxRentCharges.TextMatrix(flxRentCharges.Rows - 1, 4) = CInt(szaTemp(0))
         flxRentCharges.TextMatrix(flxRentCharges.Rows - 1, 5) = Format(txtRentStartDate.text, "dd/mm/yyyy")
         flxRentCharges.TextMatrix(flxRentCharges.Rows - 1, 6) = txtTotalRentYear.text
         flxRentCharges.TextMatrix(flxRentCharges.Rows - 1, 7) = Format(txtNextDueDate.text, "dd/mm/yyyy")
         flxRentCharges.TextMatrix(flxRentCharges.Rows - 1, 8) = txtRentDueEachPeriod.text
         flxRentCharges.TextMatrix(flxRentCharges.Rows - 1, 9) = UniqueID()
      Else
         flxRentCharges.TextMatrix(RENTCHARGES_EDIT, 0) = CByte(cboBRDemandType.ListIndex + 1)
         flxRentCharges.TextMatrix(RENTCHARGES_EDIT, 1) = cboBRDemandType.text
         flxRentCharges.TextMatrix(RENTCHARGES_EDIT, 2) = cboRentChargeDept.Column(0)
         flxRentCharges.TextMatrix(RENTCHARGES_EDIT, 3) = cboRentChargeDept.text
         flxRentCharges.TextMatrix(RENTCHARGES_EDIT, 4) = CInt(szaTemp(0))
         flxRentCharges.TextMatrix(RENTCHARGES_EDIT, 5) = Format(txtRentStartDate.text, "dd/mm/yyyy")
         flxRentCharges.TextMatrix(RENTCHARGES_EDIT, 6) = txtTotalRentYear.text
         flxRentCharges.TextMatrix(RENTCHARGES_EDIT, 7) = Format(txtNextDueDate.text, "dd/mm/yyyy")
         flxRentCharges.TextMatrix(RENTCHARGES_EDIT, 8) = txtRentDueEachPeriod.text
         flxRentCharges.TextMatrix(RENTCHARGES_EDIT, 9) = UniqueID()
      End If

''*** Saving data in the LRentCharges table.
'
'      Conn2.Connect = "DSN=" & Adsn & ";UID=;PWD="
'      Conn2.CursorDriver = rdUseIfNeeded
'      Conn2.EstablishConnection rdDriverNoPrompt
'
'      If RENTCHARGES_NEW_ENTRY Then
'         SQLStr2 = "SELECT * " & _
'                   "FROM LRentCharges;"
...
'         SQLStr2 = "SELECT * " & _
'                   "FROM LRentCharges " & _
'                   "WHERE RentCharges = '" & txtRentChargesIDEdit.text & "';"
'         Set Rst2 = Conn2.OpenResultset(SQLStr2, rdOpenDynamic, rdConcurRowVer)
'         Rst2.Edit
'      End If
'
'      Rst2!BRStartDate = CDate(Format(txtRentStartDate.text, "dd mmmm yyyy"))
'      Rst2!BRfrequency = CInt(szaTemp(0))
'      If txtTotalRentYear.text <> "" Then Rst2!BRTotal = CDbl(txtTotalRentYear.text)
'      If txtNextDueDate.text <> "" Then Rst2!BRNextDueDate = CDate(Format(txtNextDueDate.text, "dd mmmm yyyy"))
'      If txtRentDueEachPeriod.text <> "" Then Rst2!BRAmount = CDbl(txtRentDueEachPeriod.text)
'      Rst2!BRDemandType = CByte(IIf(cboBRDemandType.text <> "", cboBRDemandType.ListIndex + 1, 0))
'      Rst2!RentChargeDept = cboRentChargeDept.Column(0)
...
Private Sub flxLeaseList_Click()
   If flxLeaseList.TextMatrix(flxLeaseList.Row, 1) = "" Then Exit Sub
...
'   If cmdEditRentAnalysis.Enabled Then Exit Sub
   populateControl Me, flxRentAnalysis
   RentReviewButtonMode GridRowOnSelection
End Sub

Private Sub flxRentCharges_RowColChange()
   cboBRDemandType.ListIndex = flxRentCharges.TextMatrix(flxRentCharges.Row, 0) - 1
   cboRentChargeDept.ListIndex = flxRentCharges.TextMatrix(flxRentCharges.Row, 2) - 1
   cboFreqBR.ListIndex = flxRentCharges.TextMatrix(flxRentCharges.Row, 4) - 1
   txtRentStartDate.text = flxRentCharges.TextMatrix(flxRentCharges.Row, 5)
   txtTotalRentYear.text = flxRentCharges.TextMatrix(flxRentCharges.Row, 6)
   txtNextDueDate.text = flxRentCharges.TextMatrix(flxRentCharges.Row, 7)
   txtRentDueEachPeriod.text = flxRentCharges.TextMatrix(flxRentCharges.Row, 8)
   txtRentChargesIDEdit.text = flxRentCharges.TextMatrix(flxRentCharges.Row, 9)

   RentChargesControlsMode GridRowOnSelection
End Sub
...
Private Sub LoadFlxRentAnalysis()
   ...
   If Not Rst2.EOF Then
      ...
      While Not Rst2.EOF
         ...
         flxRentAnalysis.TextMatrix(iRow, 2) = IIf(IsNull(Rst2!RentReviewDate), "", Format(Rst2!RentReviewDate, "dd/mm/yyyy"))
         flxRentAnalysis.TextMatrix(iRow, 3) = IIf(IsNull(Rst2!RentIncreaseDate), "", Format(Rst2!RentIncreaseDate, "dd/mm/yyyy"))
         flxRentAnalysis.TextMatrix(iRow, 4) = IIf(IsNull(Rst2!RentIncreaseAmount), "", Rst2!RentIncreaseAmount)
         flxRentAnalysis.TextMatrix(iRow, 5) = Rst2!ID
         Rst2.MoveNext
...
Private Sub LoadFlxRentCharges()
   Dim iRow As Integer
...
'   get all sage account numbers and company names from tenants.
   SQLStr2 = "SELECT RentCharges, RentChargeDept, BRFrequency, BRStartDate, " & _
               "BRNextDueDate, BRTotal, BRAmount, BRDemandType, " & _
               "Type " & _
             "FROM LRentCharges, DemandTypes " & _
             "WHERE LeaseID = '" & txtLeaseID.text & "' And " & _
               "LRentCharges.BRDemandType = DemandTypes.ID " & _
   ...
   If Not Rst2.EOF Then
      iRow = 1
      While Not Rst2.EOF
         flxRentCharges.TextMatrix(iRow, 0) = IIf(IsNull(Rst2!BRDemandType), "", Rst2!BRDemandType)
         flxRentCharges.TextMatrix(iRow, 1) = IIf(IsNull(Rst2!BRDemandType), "", DemandType(Conn2, Rst2!BRDemandType))
         flxRentCharges.TextMatrix(iRow, 2) = IIf(IsNull(Rst2!RentChargeDept), "", Rst2!RentChargeDept)
         flxRentCharges.TextMatrix(iRow, 3) = IIf(IsNull(Rst2!RentChargeDept), "", DeptName(IIf(IsNull(Rst2!RentChargeDept) Or Rst2!RentChargeDept = "", 1, Rst2!RentChargeDept)))
         flxRentCharges.TextMatrix(iRow, 4) = IIf(IsNull(Rst2!BRfrequency), "", Rst2!BRfrequency)
         flxRentCharges.TextMatrix(iRow, 5) = IIf(IsNull(Rst2!BRStartDate), "", Format(Rst2!BRStartDate, "dd/mm/yyyy"))
         flxRentCharges.TextMatrix(iRow, 6) = Rst2!BRTotal
         flxRentCharges.TextMatrix(iRow, 7) = IIf(IsNull(Rst2!BRNextDueDate), "", Format(Rst2!BRNextDueDate, "dd/mm/yyyy"))
         flxRentCharges.TextMatrix(iRow, 8) = Rst2!BRAmount
         flxRentCharges.TextMatrix(iRow, 9) = Rst2!RentCharges
         Rst2.MoveNext
         If Not Rst2.EOF Then flxRentCharges.AddItem ""
         iRow = iRow + 1
      Wend
   End If
...
Private Sub ConfigureFlxRentAnalysis()
   Dim szFlxHeader As String
   
   flxRentAnalysis.RowHeight(0) = 150
   flxRentAnalysis.Clear
   flxRentAnalysis.Cols = 6
   ...
   flxRentAnalysis.ColWidth(1) = txtRentReviewDate.Left - txtSerial.Left
   flxRentAnalysis.ColWidth(2) = txtRentIncreaseDate.Left - txtRentReviewDate.Left
   flxRentAnalysis.ColWidth(3) = txtRentIncreaseAmount.Left - txtRentIncreaseDate.Left
   flxRentAnalysis.ColWidth(4) = txtRentIncreaseAmount.Width
   flxRentAnalysis.ColWidth(5) = 0       'ID
End Sub

Private Sub ConfigureFlxRentCharges()
   Dim szFlxHeader As String

   flxRentCharges.RowHeight(0) = 150
   flxRentCharges.Clear
   flxRentCharges.Rows = 2
   flxRentCharges.Cols = 10
   szFlxHeader$ = "BRDemandTypeID|<BRDemandType|<RentChargeDeptID|<RentChargeDept|<FreqBR|<RentStartDate|>TotalRentYear|<NextDueDate|>RentDueEachPeriod|<LeaseIDEdit"
   flxRentCharges.FormatString = szFlxHeader$

   flxRentCharges.ColWidth(0) = 0
   flxRentCharges.ColWidth(1) = cboRentChargeDept.Left - cboBRDemandType.Left
   flxRentCharges.ColWidth(2) = 0
   flxRentCharges.ColWidth(3) = cboFreqBR.Left - cboRentChargeDept.Left
   flxRentCharges.ColWidth(4) = txtRentStartDate.Left - cboFreqBR.Left
   flxRentCharges.ColWidth(5) = txtTotalRentYear.Left - txtRentStartDate.Left
   flxRentCharges.ColWidth(6) = txtNextDueDate.Left - txtTotalRentYear.Left
   flxRentCharges.ColWidth(7) = txtRentDueEachPeriod.Left - txtNextDueDate.Left
   flxRentCharges.ColWidth(8) = txtRentDueEachPeriod.Width - 5
   flxRentCharges.ColWidth(9) = 0
End Sub
...
'      cboRentPayable.text = "No"
   ...
   '  Rent Review tab
   '****************************************
   If IsNull(Rst1!RentReviewDate) = False Then txtRentReviewDt.text = Format(Rst1!RentReviewDate, "dd/mm/yyyy")
   If IsNull(Rst1!RentIncreaseDate) = False Then txtRentIncDt.text = Format(Rst1!RentIncreaseDate, "dd/mm/yyyy")
   ...
   '  Supplementary tab
   '****************************************
   If IsNull(Rst1!DateFlagDate) = False Then txtDtFlgDate.text = Format(Rst1!DateFlagDate, "dd/mm/yyyy")
   ...
   If Rst1!InsurancePayable = "Y" Then
      ...
      txtInsuranceStartDate.text = IIf(IsNull(Rst1!InsuranceStartDate), "", Format(Rst1!InsuranceStartDate, "dd/mm/yyyy"))
      txtInsuranceEndDate.text = IIf(IsNull(Rst1!InsuranceEndDate), "", Format(Rst1!InsuranceEndDate, "dd/mm/yyyy"))
      cboInsuranceDemandType.ListIndex = IIf(IsNull(Rst1!InsuranceDemandType), "", CByte(Rst1!InsuranceDemandType) - 1)
      txtInsuranceNextDueDate.text = IIf(IsNull(Rst1!InsuranceNextDueDate), "", Format(Rst1!InsuranceNextDueDate, "dd/mm/yyyy"))
...
Private Sub optGlobalData_Click()
   ...
   If cmdSaveNew.Visible Or cmdSaveEdit.Visible Then txtGlobalAmount.SetFocus

   Dim rdoConn As New RDO.rdoConnection
   Dim rstRst As rdoResultset
   Dim szSQL As String, szaUnit() As String, lArea As Long
   ...
   lArea = GetUnitTA(rdoConn, szaUnit(0))
   If lArea = 0 Then
      ...
      txtGlobalAmount.Locked = True
      optGlobalData.Enabled = False
   Else
      txtGlobalAmount.text = Format(GetPPSF(rdoConn, szaUnit(0)) * lArea, "0.00")
      ...
      If cmdSaveNew.Visible Then cmdSaveNew.SetFocus
      If cmdSaveEdit.Visible Then cmdSaveEdit.SetFocus
   End If
...
Private Sub tabLease_Click(PreviousTab As Integer)
   ...
   Select Case tabLease.Tab
   Case 3:
      If cboBreakClause.text = "No" Then
         Frame1(0).Enabled = False
         cboBreakClause.SetFocus
      End If
   Case 4:
      If cboSCPayable.text = "No" Then
         Frame1(2).Enabled = False
         cboSCPayable.SetFocus
      End If
   Case 5:
      If cboIntCrgable.text = "No" Then
         Frame3.Enabled = False
         cboIntCrgable.SetFocus
      End If
   Case 8:
      If cboInsurancePayable.text = "No" Then
         fmeInsurance.Enabled = False
         cboInsurancePayable.SetFocus
      End If
   End Select
End Sub
...
Private Sub txtInsuranceEndDate_GotFocus()
   If txtInsuranceStartDate.text = "" Then
      MsgBox "Please set the start date first.", vbInformation + vbOKOnly, "Insurance"
      txtInsuranceStartDate.SetFocus
   End If
End Sub
...
Private Sub txtRentStartDate_GotFocus()
   If cboFreqBR.text = "" Then
      MsgBox "Please select the Frequency befor set the start date.", vbCritical + vbOKOnly, "Frequency"
      txtRentStartDate.text = ""
   End If
End Sub
...
Private Sub txtTotalRentYear_GotFocus()
   If cboFreqBR.text = "" Then
      MsgBox "Please select the Frequency befor input the total.", vbCritical + vbOKOnly, "Frequency"
      txtTotalRentYear.text = ""
   End If
End Sub
...
Private Sub txtAnnualService_KeyPress(KeyAscii As Integer)
   If (KeyAscii < 48 Or KeyAscii > 57) And KeyAscii <> 46 And KeyAscii <> 8 And KeyAscii <> 13 Then
      KeyAscii = 0
      Exit Sub
   Else
      If KeyAscii = 13 Then
...
If txtInsuranceEndDate.text = "" Then Exit Sub
'Added By Asif. 13/01/2006
TextBoxFormatDate txtInsuranceEndDate
If DateDiff("d", CDate(txtInsuranceStartDate.text), CDate(txtInsuranceEndDate.text)) < 0 Then
   MsgBox "End date can not be the previous date of the start date.", vbCritical + vbOKOnly, "Insurance"
   txtInsuranceEndDate.text = ""
   txtInsuranceStartDate.SelStart = 1
   txtInsuranceStartDate.SelLength = Len(txtInsuranceStartDate.text)
End If
End Sub
...
Private Sub txtPPSqFoot_LostFocus()
   ...
   Area = GetUnitTA(Conn1, Left(cboUnit.text, 8))
...
' Modified by Samrat 08/02/2006
If txtLeaseEndDate.text <> "" Then
   TextBoxFormatDate txtLeaseEndDate
Else
   Exit Sub
End If

If DateDiff("d", txtLeaseStDt.text, txtLeaseEndDate.text) < 0 Then
   MsgBox "Lease Start date can not be after Lease End date.", vbCritical + vbOKOnly, "Date Error"
   txtLeaseEndDate.text = ""
   Exit Sub
End If
If DateDiff("d", Now, txtLeaseEndDate.text) < 0 Then
   MsgBox "This lease will be saved as expired because of the Lease End date, which is past.", vbInformation + vbOKOnly, "Lease Expired"
   txtLeaseEndDate.SelStart = 0
   txtLeaseEndDate.SelLength = Len(txtLeaseEndDate.text)
End If
End Sub
...
Private Sub txtTotalRentYear_LostFocus()
   If txtTotalRentYear.text <> "" Then
      If Not IsNumeric(txtTotalRentYear.text) Then
         ...
         txtTotalRentYear.SelStart = 0
         txtTotalRentYear.SelLength = Len(txtTotalRentYear.text)
         Exit Sub
       Else
...
Public Sub RentNextDueDate()
   ...
   Select Case cboFreqBR.text
      ...
      Case cboFreqBR.List(3):                               'Fortnightly in arrears
         txtNextDueDate.text = DateAdd("d", 14, txtRentStartDate.text)
      Case cboFreqBR.List(4):                               'Monthly in advance
         txtNextDueDate.text = NextPayingDate(txtRentStartDate.text, InAdv, Pay_Monthly)
      Case cboFreqBR.List(5):                               'Monthly in arrears
         txtNextDueDate.text = NextPayingDate(txtRentStartDate.text, InArr, Pay_Monthly)
      Case cboFreqBR.List(6):                               'Quarterly in advance
         txtNextDueDate.text = NextPayingDate(txtRentStartDate.text, InAdv, Pay_Quarterly)
      Case cboFreqBR.List(7):                               'Quarterly in arrears
         txtNextDueDate.text = NextPayingDate(txtRentStartDate.text, InArr, Pay_Quarterly)
      Case cboFreqBR.List(8):                               'Half yearly in advance
         txtNextDueDate.text = NextPayingDate(txtRentStartDate.text, InAdv, Pay_Half_Yearly)
      Case cboFreqBR.List(9):                               'Half yearly in arrears
         txtNextDueDate.text = NextPayingDate(txtRentStartDate.text, InArr, Pay_Half_Yearly)
      Case cboFreqBR.List(10):                              'yearly in advance
         txtNextDueDate.text = NextPayingDate(txtRentStartDate.text, InAdv, Pay_Yearly)
      Case cboFreqBR.List(11):                              'yearly in arrears
         txtNextDueDate.text = NextPayingDate(txtRentStartDate.text, InArr, Pay_Yearly)
   End Select
End Sub

Public Sub RentNextDueDate_OLD()
Dim xy As Integer
Dim xm As Integer
Dim xd As Integer
Dim qy1 As Integer
...
'If txtTotalRentYear.text <> "" Then 'If there is a Base Rate for Year Figure
    Select Case cboFreqBR.text
        Case cboFreqBR.List(0): ' Weekly in advance
            txtNextDueDate.text = Format(txtRentStartDate.text, "dd/mm/yyyy")
        Case cboFreqBR.List(1): ' Weekly in arrears
            txtNextDueDate.text = Format(DateAdd("d", 7, txtRentStartDate.text), "dd/mm/yyyy")
        Case cboFreqBR.List(2): ' Fortnightly in advance
            txtNextDueDate.text = Format(txtRentStartDate.text, "dd/mm/yyyy")
        Case cboFreqBR.List(3): ' Fortnightly in arrears
            txtNextDueDate.text = Format(DateAdd("d", 7, txtRentStartDate.text), "dd/mm/yyyy")
        Case cboFreqBR.List(4): ' Monthly in advance
            txtNextDueDate.text = Format(txtRentStartDate.text, "dd/mm/yyyy")
        Case cboFreqBR.List(5): ' Monthly in arrears
            txtNextDueDate.text = Format(DateAdd("m", 1, txtRentStartDate.text), "dd/mm/yyyy")
        Case cboFreqBR.List(6): ' Quarterly in advance
            Select Case xm
                Case Is < qm2:
                    Select Case xm
                        Case Is < qm1:
                            txtNextDueDate.text = Format(quarterly1, "dd/mm/yyyy")
                        Case qm1:
                            Select Case xd
                                Case Is < qd1:
                                    txtNextDueDate.text = Format(quarterly1, "dd/mm/yyyy")
                                Case qd1:
                                    txtNextDueDate.text = Format(quarterly1, "dd/mm/yyyy")
                                Case Is > qd1:
                                    txtNextDueDate.text = Format(quarterly2, "dd/mm/yyyy")
                            End Select
                        Case Is > qm1:
                            txtNextDueDate.text = Format(quarterly2, "dd/mm/yyyy")
                    End Select
                Case qm2:
                    Select Case xd
                        ...
                        Case qd2:
                            txtNextDueDate.text = Format(quarterly2, "dd/mm/yyyy")
                        Case Is > qd2:
                            txtNextDueDate.text = Format(quarterly3, "dd/mm/yyyy")
                    End Select
                Case Is > qm2:
                    Select Case xm
                        Case Is < qm3:
                        ...
                        Case qm3:
                            Select Case xd
                                ...
                                Case qd3:
                                    txtNextDueDate.text = Format(quarterly3, "dd/mm/yyyy")
                                Case Is > qd3:
                                    txtNextDueDate.text = Format(quarterly4, "dd/mm/yyyy")
                            End Select
                        Case Is > qm3:
                            Select Case xm
                                Case Is < qm4:
                                ...
                                Case qm4:
                                    Select Case xd
                                        ...
                                        Case qd4:
                                            txtNextDueDate.text = Format(quarterly4, "dd/mm/yyyy")
                                        Case Is > qd4:
                                            txtNextDueDate.text = Format(DateAdd("yyyy", 1, quarterly1), "dd/mm/yyyy")
                                    End Select
                                Case Is > qm4:
                                   txtNextDueDate.text = Format(DateAdd("yyyy", 1, quarterly1), "dd/mm/yyyy")
                            End Select
                                    ...
                                    Case qm4:
                                        Select Case xd
                                            ...
                                            Case qd4:
                                                txtNextDueDate.text = Format(DateAdd("yyyy", 1, quarterly1), "dd/mm/yyyy")
                                            Case Is > qd4:
                                                txtNextDueDate.text = Format(DateAdd("yyyy", 1, quarterly2), "dd/mm/yyyy")
                                        End Select
                                    Case Is > qm4:
                                        txtNextDueDate.text = Format(DateAdd("yyyy", 1, quarterly2), "dd/mm/yyyy")
                                End Select
        ...
        Case cboFreqBR.List(8): ' Half yearly in advance
            Select Case xm
                Case Is < hm1:
                    txtNextDueDate.text = Format(halfyearly1, "dd/mm/yyyy")
                Case hm1:
                    Select Case xd
                        Case Is < hd1:
                            txtNextDueDate.text = Format(halfyearly1, "dd/mm/yyyy")
                        Case hd1:
                            txtNextDueDate.text = Format(halfyearly1, "dd/mm/yyyy")
                        Case Is > hd1:
                            txtNextDueDate.text = Format(halfyearly2, "dd/mm/yyyy")
                    End Select
                Case Is > hm1:
                    Select Case xm:
                        Case Is < hm2:
                        ...
                        Case hm2:
                            Select Case xd
                                ...
                                Case hd2:
                                    txtNextDueDate.text = Format(halfyearly2, "dd/mm/yyyy")
                                Case Is > hd2:
                                    txtNextDueDate.text = Format(DateAdd("yyyy", 1, halfyearly1), "dd/mm/yyyy")
                            End Select
                        Case Is > hm2:
                            txtNextDueDate.text = Format(DateAdd("yyyy", 1, halfyearly1), "dd/mm/yyyy")
                    End Select
                ...
                Case hm2:
                    Select Case xd
                        ...
                        Case hd2:
                            txtNextDueDate.text = Format(DateAdd("yyyy", 1, halfyearly1), "dd/mm/yyyy")
                        Case Is > hd2:
                            txtNextDueDate.text = Format(DateAdd("yyyy", 1, halfyearly2), "dd/mm/yyyy")
                    End Select
                Case Is > hm2:
                    txtNextDueDate.text = Format(DateAdd("yyyy", 1, halfyearly2), "dd/mm/yyyy")
            End Select
        ...
        Case cboFreqBR.List(11): ' Yearly in arrears
            txtNextDueDate.text = Format(DateAdd("yyyy", 1, txtRentStartDate.text), "dd/mm/yyyy")
    End Select
...
If txtNextDueDate.text <> "" And txtRentStartDate.text <> "" Then
    ...
    If b <> 0 Then
        txtNextDueDate.text = Format(DateAdd("yyyy", b, txtNextDueDate.text), "dd/mm/yyyy")
    End If
End If
...
Public Sub SetNextDueDtSC()
   ...
   Select Case cboFreqSC.text
      ...
      Case cboFreqSC.List(4):                               'Monthly in advance
         txtSCNextDueDt.text = NextPayingDate(txtPayableFrom.text, InAdv, Pay_Monthly)
      Case cboFreqSC.List(5):                               'Monthly in arrears
         txtSCNextDueDt.text = NextPayingDate(txtPayableFrom.text, InArr, Pay_Monthly)
      Case cboFreqSC.List(6):                               'Quarterly in advance
         txtSCNextDueDt.text = NextPayingDate(txtPayableFrom.text, InAdv, Pay_Quarterly)
      Case cboFreqSC.List(7):                               'Quarterly in arrears
         txtSCNextDueDt.text = NextPayingDate(txtPayableFrom.text, InArr, Pay_Quarterly)
      Case cboFreqSC.List(8):                               'Half yearly in advance
         txtSCNextDueDt.text = NextPayingDate(txtPayableFrom.text, InAdv, Pay_Half_Yearly)
      Case cboFreqSC.List(9):                               'Half yearly in arrears
         txtSCNextDueDt.text = NextPayingDate(txtPayableFrom.text, InArr, Pay_Half_Yearly)
      Case cboFreqSC.List(10):                              'yearly in advance
         txtSCNextDueDt.text = NextPayingDate(txtPayableFrom.text, InAdv, Pay_Yearly)
      Case cboFreqSC.List(11):                              'yearly in arrears
         txtSCNextDueDt.text = NextPayingDate(txtPayableFrom.text, InArr, Pay_Yearly)
   End Select
End Sub

Public Sub SetNextDueDtSC_OLD()
'   Dim iPayableYear As Integer
'   Dim iPayableMth  As Integer
'   Dim iPayableDay As Integer
'   Dim i1st_Qt_Yrs As Integer
'   Dim i2nd_Qt_Yrs As Integer
'   Dim i3rd_Qt_Yrs As Integer
'   Dim i4th_Qt_Yrs As Integer
'   Dim i1st_Qt_Mth As Integer
'   Dim i2nd_Qt_Mth As Integer
'   Dim i3rd_Qt_Mth As Integer
'   Dim i4th_Qt_Mth As Integer
'   Dim i1st_Qt_Day As Integer
'   Dim i2nd_Qt_Day As Integer
'   Dim i3rd_Qt_Day As Integer
'   Dim i4th_Qt_Day As Integer
'   Dim i1st_Hf_Yrs As Integer
'   Dim i2nd_Hf_Yrs As Integer
'   Dim i1st_Hf_Mth As Integer
'   Dim i2nd_Hf_Mth As Integer
'   Dim i1st_Hf_Day As Integer
'   Dim i2nd_Hf_Day As Integer
'   Dim iYr_Yrs As Integer
'   Dim iYr_Mth As Integer
'   Dim iYr_Day As Integer
'   Dim b As Integer
'
'   iPayableYear = CInt(Right(txtPayableFrom.text, 2))
'   iPayableMth = CInt(Mid(txtPayableFrom.text, 4, 2))
'   iPayableDay = CInt(Left(txtPayableFrom.text, 2))
'   i1st_Qt_Day = CInt(Left(quarterly1, 2))
'   i2nd_Qt_Day = CInt(Left(quarterly2, 2))
'   i3rd_Qt_Day = CInt(Left(quarterly3, 2))
'   i4th_Qt_Day = CInt(Left(quarterly4, 2))
'   i1st_Qt_Mth = CInt(Mid(quarterly1, 4, 2))
'   i2nd_Qt_Mth = CInt(Mid(quarterly2, 4, 2))
'   i3rd_Qt_Mth = CInt(Mid(quarterly3, 4, 2))
'   i4th_Qt_Mth = CInt(Mid(quarterly4, 4, 2))
'   i1st_Qt_Yrs = Right(Year(Date), 2)
'   i2nd_Qt_Yrs = Right(Year(Date), 2)
'   i3rd_Qt_Yrs = Right(Year(Date), 2)
'   i4th_Qt_Yrs = Right(Year(Date), 2)
'
'   i1st_Hf_Day = CInt(Left(halfyearly1, 2))
'   i2nd_Hf_Day = CInt(Left(halfyearly2, 2))
'   i1st_Hf_Mth = CInt(Mid(halfyearly1, 4, 2))
'   i2nd_Hf_Mth = CInt(Mid(halfyearly2, 4, 2))
'   i1st_Hf_Yrs = Right(Year(Date), 2)
'   i2nd_Hf_Yrs = Right(Year(Date), 2)
'
'   iYr_Day = CInt(Left(yearly, 2))
'   iYr_Mth = CInt(Mid(yearly, 4, 2))
'   iYr_Yrs = Right(Year(Date), 2)
...
'   txtSCNextDueDt.Enabled = True
   ...
   Select Case cboFreqSC.text
      ...
      Case cboFreqSC.List(7):                               'Quarterly in arrears
         txtSCNextDueDt.text = NextPayingDate(txtPayableFrom.text, InArr, Pay_Quarterly)
'          Select Case iPayableMth
'              Case Is < i2nd_Qt_Mth:
'                  Select Case iPayableMth
'                      Case Is < i1st_Qt_Mth:
'                          txtSCNextDueDt.text = Format(quarterly1, "dd/mm/yyyy")
'                      Case i1st_Qt_Mth:
'                          Select Case iPayableDay
'                              Case Is < i1st_Qt_Day:
'                                  txtSCNextDueDt.text = Format(quarterly1, "dd/mm/yyyy")
'                              Case i1st_Qt_Day:
'                                  txtSCNextDueDt.text = Format(quarterly1, "dd/mm/yyyy")
'                              Case Is > i1st_Qt_Day:
'                                  txtSCNextDueDt.text = Format(quarterly2, "dd/mm/yyyy")
'                          End Select
'                      Case Is > i1st_Qt_Mth:
'                          txtSCNextDueDt.text = Format(quarterly2, "dd/mm/yyyy")
'                  End Select
...
'                          txtSCNextDueDt.text = Format(quarterly2, "dd/mm/yyyy")
'                      Case Is > i2nd_Qt_Day:
'                          txtSCNextDueDt.text = Format(quarterly3, "dd/mm/yyyy")
'                  End Select
'              Case Is > i2nd_Qt_Mth:
'                  Select Case iPayableMth
'                      Case Is < i3rd_Qt_Mth:
...
'                                  txtSCNextDueDt.text = Format(quarterly3, "dd/mm/yyyy")
'                              Case Is > i3rd_Qt_Day:
'                                  txtSCNextDueDt.text = Format(quarterly4, "dd/mm/yyyy")
'                          End Select
'                      Case Is > i3rd_Qt_Mth:
'                          Select Case iPayableMth
'                              Case Is < i4th_Qt_Mth:
'                                  txtSCNextDueDt.text = Format(quarterly4, "dd/mm/yyyy") 'i4th_Qt_Mth
'                              Case i4th_Qt_Mth:
'                                  Select Case iPayableDay
'                                      Case Is < i4th_Qt_Day:
'                                          txtSCNextDueDt.text = Format(quarterly4, "dd/mm/yyyy")
...
'                                          txtSCNextDueDt.text = Format(quarterly4, "dd/mm/yyyy")
'                                      Case Is > i4th_Qt_Day:
'                                          txtSCNextDueDt.text = DateAdd("yyyy", 1, Format(quarterly1, "dd/mm/yyyy"))
'                                  End Select
'                              Case Is > i4th_Qt_Mth:
'                                 txtSCNextDueDt.text = DateAdd("yyyy", 1, Format(quarterly1, "dd/mm/yyyy"))
'                          End Select
...
'                                         txtSCNextDueDt.text = DateAdd("yyyy", 1, Format(quarterly1, "dd/mm/yyyy"))
'                                     Case Is > i4th_Qt_Day:
'                                         txtSCNextDueDt.text = DateAdd("yyyy", 1, Format(quarterly2, "dd/mm/yyyy"))
'                                 End Select
'                             Case Is > i4th_Qt_Mth:
'                                 txtSCNextDueDt.text = DateAdd("yyyy", 1, Format(quarterly2, "dd/mm/yyyy"))
'                         End Select
...
'         End Select
'
''Different way to do by calling function. Finally I have changed it through calling Module
''samrat 20.06.2006 modNextDueDate
'         Dim iTemp As Integer
'         iTemp = IsQuarterDate(CDate(txtPayableFrom.text))
'         If iTemp > 0 Then
'            Select Case iTemp
'               Case 1:
'                  txtSCNextDueDt.text = Format(quarterly2, "dd/mm/yyyy")
...
'            End Select
'         Else
'            txtSCNextDueDt.text = Format(InQuarterDate(CDate(txtPayableFrom.text)), "dd/mm/yyyy")
'         End If
...
'         Select Case iPayableMth
'            Case Is < i1st_Hf_Mth:
'               txtSCNextDueDt.text = Format(halfyearly1, "dd/mm/yyyy")
'            Case i1st_Hf_Mth:
'               Select Case iPayableDay
'                  Case Is < i1st_Hf_Day:
'                     txtSCNextDueDt.text = Format(halfyearly1, "dd/mm/yyyy")
...
'                     txtSCNextDueDt.text = Format(halfyearly1, "dd/mm/yyyy")
'                  Case Is > i1st_Hf_Day:
'                     txtSCNextDueDt.text = Format(halfyearly2, "dd/mm/yyyy")
'               End Select
'            Case Is > i1st_Hf_Mth:
'               Select Case iPayableMth:
'                  Case Is < i2nd_Hf_Mth:
...
'                            txtSCNextDueDt.text = Format(halfyearly2, "dd/mm/yyyy")
'                        Case Is > i2nd_Hf_Day:
'                            txtSCNextDueDt.text = Format(DateAdd("yyyy", 1, halfyearly1), "dd/mm/yyyy")
'                     End Select
'                  Case Is > i2nd_Hf_Mth:
'                     txtSCNextDueDt.text = Format(DateAdd("yyyy", 1, halfyearly1), "dd/mm/yyyy")
'               End Select
...
'                     txtSCNextDueDt.text = Format(DateAdd("yyyy", 1, halfyearly1), "dd/mm/yyyy")
'                  Case Is > i2nd_Hf_Day:
'                     txtSCNextDueDt.text = Format(DateAdd("yyyy", 1, halfyearly2), "dd/mm/yyyy")
'               End Select
'            Case Is > i2nd_Hf_Mth:
'                txtSCNextDueDt.text = Format(DateAdd("yyyy", 1, halfyearly2), "dd/mm/yyyy")
'         End Select
...
Public Sub CalculateInsuranceCharge()
   ...
   Select Case cboInsuranceFrequency.text
      ...
      Case cboInsuranceFrequency.List(4):                               'Monthly in advance
         txtInsuranceNextDueDate.text = NextPayingDate(txtInsuranceStartDate.text, InAdv, Pay_Monthly)
      Case cboInsuranceFrequency.List(5):                               'Monthly in arrears
         txtInsuranceNextDueDate.text = NextPayingDate(txtInsuranceStartDate.text, InArr, Pay_Monthly)
      Case cboInsuranceFrequency.List(6):                               'Quarterly in advance
         txtInsuranceNextDueDate.text = NextPayingDate(txtInsuranceStartDate.text, InAdv, Pay_Quarterly)
      Case cboInsuranceFrequency.List(7):                               'Quarterly in arrears
         txtInsuranceNextDueDate.text = NextPayingDate(txtInsuranceStartDate.text, InArr, Pay_Quarterly)
      Case cboInsuranceFrequency.List(8):                               'Half yearly in advance
         txtInsuranceNextDueDate.text = NextPayingDate(txtInsuranceStartDate.text, InAdv, Pay_Half_Yearly)
      Case cboInsuranceFrequency.List(9):                               'Half yearly in arrears
         txtInsuranceNextDueDate.text = NextPayingDate(txtInsuranceStartDate.text, InArr, Pay_Half_Yearly)
      Case cboInsuranceFrequency.List(10):                              'yearly in advance
         txtInsuranceNextDueDate.text = NextPayingDate(txtInsuranceStartDate.text, InAdv, Pay_Yearly)
      Case cboInsuranceFrequency.List(11):                              'yearly in arrears
         txtInsuranceNextDueDate.text = NextPayingDate(txtInsuranceStartDate.text, InArr, Pay_Yearly)
   End Select
End Sub

Public Sub CalculateInsuranceCharge_OLD()
   If cboUnit.text = "" Then
       MsgBox "You must select a unit!", vbOKOnly + vbCritical, "No Unit Selected"
       Exit Sub
   End If
...
Private Sub txtSCPercentage_LostFocus()
   ...
   Dim Total As Double, TotalServiceCharge As String, temp() As String
   ...
   TotalServiceCharge = GetGlobalTotalSC(Conn1, Left(cboUnit, 8))
...
Private Sub RentChargesControlsMode(ByVal mode As ComponentMode)
   Select Case mode
      Case ComponentMode.DefaultMode
         cboRentChargeDept.text = ""
         cboRentChargeDept.Locked = True
         cboFreqBR.text = ""
         cboFreqBR.Locked = True
         txtRentStartDate.text = ""
         ...
         txtRentDueEachPeriod.text = ""
         txtRentDueEachPeriod.Locked = True
         cboBRDemandType.text = ""
         cboBRDemandType.Locked = True
      ...
      Case ComponentMode.EditMode
         cboRentChargeDept.Locked = False
         cboFreqBR.Locked = False
         txtRentStartDate.Locked = False
'         txtNextDueDate.Locked = False
         txtTotalRentYear.Locked = False
'         txtRentDueEachPeriod.Locked = False
         cboBRDemandType.Locked = False
...
Error_Handler:
      ...
      MsgBox "(frmLease3 LoadDept) The SDO generated the following error: " & oSDO.LastError.text
   Set oDepartmentData = Nothing
   Set oWS = Nothing
   Set oSDO = Nothing
End Sub
...
Public Sub EmptyBoxes()
   ...
   optSqFoot.Enabled = True
   optGlobalData.Enabled = True
